...
def get_temperature_edit_string_adapt(temperature_init, temperature_final, data_fraction, iter_):
...
def get_temperature_edit_string(data_fraction, iter_):
    ...
    temperature_proportions = [('*', (1-data_fraction) * (1 - 0.03) + 0.03)]
    # temperature_proportions = [('*', 1.0)]

    edit_config_lines = []
    dropout_info = []

    for component_name, temp_proportion in temperature_proportions:
        edit_config_lines.append(
            "set-temperature-proportion name={0} proportion={1}".format(
                component_name, temp_proportion))
        # temp_info.append("pattern/temperature-proportion={0}/{1}".format(
        #     component_name, temp_proportion))

    # if _debug_temp:
    #     logger.info("On iteration %d, %s", iter_, ', '.join(temp_info))
    return ("""nnet3-copy --edits='{edits}' - - |""".format(
        edits=";".join(edit_config_lines)))
