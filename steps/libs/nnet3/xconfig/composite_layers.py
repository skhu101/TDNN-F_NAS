...
class XconfigTdnnfDARTSLayer(XconfigLayerBase):

    def __init__(self, first_token, key_to_value, prev_names = None):
        assert first_token == "tdnnfdarts-layer"
        XconfigLayerBase.__init__(self, first_token, key_to_value, prev_names)
    ...
    def _generate_config(self):
        ...
        # The first linear layer, from input-dim (spliced x2) to bottleneck-dim
        configs.append('component name={0}.linear type=TdnnDARTSComponent input-dim={1} '
                       'output-dim={2} l2-regularize={3} max-change={4} use-bias=false '
                       'time-offsets={5} orthonormal-constraint=-1.0'.format(
                           name, input_dim, bottleneck_dim, l2_regularize,
                           max_change, time_offsets1))
        ...
        # The affine layer, from bottleneck-dim (spliced x2) to output-dim
        configs.append('component name={0}.affine type=TdnnDARTSComponent '
                       'input-dim={1} output-dim={2} l2-regularize={3} max-change={4} '
                       'time-offsets={5}'.format(
                           name, bottleneck_dim, output_dim, l2_regularize,
                           max_change, time_offsets2))
...
# This is passed through to the linear and affine components.  You'll normally
# want this to be set to a nonzero value, e.g. 0.004.
class XconfigTdnnfDARTSV2Layer(XconfigLayerBase):

    def __init__(self, first_token, key_to_value, prev_names = None):
        assert first_token == "tdnnfdartsv2-layer"
        XconfigLayerBase.__init__(self, first_token, key_to_value, prev_names)
    ...
    def _generate_config(self):
        ...
        max_change = self.config['max-change']
        self_repair_scale = self.config['self-repair-scale']
        use_gumbel = self.config['use-gumbel']
        use_entropy = self.config['use-entropy']
        free_select = self.config['free-select']

        # The first linear layer, from input-dim (spliced x2) to bottleneck-dim
        configs.append('component name={0}.linear type=TdnnDARTSV2Component input-dim={1} '
                       'output-dim={2} l2-regularize={3} max-change={4} use-bias=false '
                        'use-gumbel={5} use-entropy={6} free-select={7} Temp-Proportion=1.0 time-offsets={8} orthonormal-constraint=-1.0'.format(
                           name, input_dim, bottleneck_dim, l2_regularize,
                           max_change, use_gumbel, use_entropy, free_select, time_offsets1))
        configs.append('component-node name={0}.linear component={0}.linear '
                       'input={1}'.format(name, input_descriptor))

        # The affine layer, from bottleneck-dim (spliced x2) to output-dim
        configs.append('component name={0}.affine type=TdnnDARTSV2Component '
                       'input-dim={1} output-dim={2} l2-regularize={3} max-change={4} '
                       'use-gumbel={5} use-entropy={6} free-select={7} Temp-Proportion=1.0 time-offsets={8}'.format(
                           name, bottleneck_dim, output_dim, l2_regularize,
                           max_change, use_gumbel, use_entropy, free_select, time_offsets2))
        configs.append('component-node name={0}.affine component={0}.affine '
                       'input={0}.linear'.format(name))
...
class XconfigTdnnfDARTSV3Layer(XconfigLayerBase):

    def __init__(self, first_token, key_to_value, prev_names = None):
        assert first_token == "tdnnfdartsv3-layer"
        XconfigLayerBase.__init__(self, first_token, key_to_value, prev_names)
    ...
    def _generate_config(self):
        ...
        use_entropy = self.config['use-entropy']
        free_select = self.config['free-select']
        update_alpha = self.config['update-alpha']
        update_theta = self.config['update-theta']
        uniform_sample = self.config['uniform-sample']

        # The first linear layer, from input-dim (spliced x2) to bottleneck-dim
        configs.append('component name={0}.linear type=TdnnDARTSV3Component input-dim={1} '
                       'output-dim={2} l2-regularize={3} max-change={4} use-bias=false '
                        'use-gumbel={5} use-entropy={6} free-select={7} update-alpha={8} update-theta={9} uniform-sample={10} Temp-Proportion=1.0 time-offsets={11} orthonormal-constraint=-1.0'.format(
                           name, input_dim, bottleneck_dim, l2_regularize,
                           max_change, use_gumbel, use_entropy, free_select, update_alpha, update_theta, uniform_sample, time_offsets1))
        configs.append('component-node name={0}.linear component={0}.linear '
                       'input={1}'.format(name, input_descriptor))

        # The affine layer, from bottleneck-dim (spliced x2) to output-dim
        configs.append('component name={0}.affine type=TdnnDARTSV3Component '
                       'input-dim={1} output-dim={2} l2-regularize={3} max-change={4} '
                       'use-gumbel={5} use-entropy={6} free-select={7} update-alpha={8} update-theta={9} uniform-sample={10} Temp-Proportion=1.0 time-offsets={11}'.format(
                           name, bottleneck_dim, output_dim, l2_regularize,
                           max_change, use_gumbel, use_entropy, free_select, update_alpha, update_theta, uniform_sample, time_offsets2))
        configs.append('component-node name={0}.affine component={0}.affine '
                       'input={0}.linear'.format(name))
...
class XconfigBayesTdnnfLayer(XconfigLayerBase):

    def __init__(self, first_token, key_to_value, prev_names = None):
        assert first_token == "bayestdnnf-layer"
        XconfigLayerBase.__init__(self, first_token, key_to_value, prev_names)
    ...
    def set_default_configs(self):
        self.config = {'input':'[-1]',
                       ...
                       'test-flag': 'true',
                       'use-exp-std': 'true',
                       'share-std-output-sampling': 'true',
                       'KL-scale': 1e-4, }
    ...
    def _generate_config(self):
        ...
        test_flag = self.config['test-flag']
        use_exp_std = self.config['use-exp-std']
        share_std_output_sampling = self.config['share-std-output-sampling']
        ...
        # The affine layer, from bottleneck-dim (spliced x2) to output-dim
        configs.append('component name={0}.affine type=BayesTdnnComponent '
                       'input-dim={1} output-dim={2} l2-regularize={3} max-change={4} '
                       'time-offsets={5} test-flag={6} use-exp-std={7} share-std-output-sampling={8} KL-scale={9}'.format(
                           name, bottleneck_dim, output_dim, l2_regularize,
                           max_change, time_offsets2, test_flag, use_exp_std, share_std_output_sampling, KL_scale))
        configs.append('component-node name={0}.affine component={0}.affine '
                       'input={0}.linear'.format(name))
...
class XconfigGPTdnnfLayer(XconfigLayerBase):

    def __init__(self, first_token, key_to_value, prev_names = None):
        assert first_token == "gptdnnf-layer"
        XconfigLayerBase.__init__(self, first_token, key_to_value, prev_names)
    ...
    def _generate_config(self):
        ...
        test_flag = self.config['test-flag']
        gpact_test_mode = self.config['gpact-test-mode']
        use_exp_std = self.config['use-exp-std']
        share_std_output_sampling = self.config['share-std-output-sampling']
        KL_scale = self.config['KL-scale']
        gpact_KL_scale = self.config['gpact-KL-scale']
        prior_std = self.config['prior-std']
        ...
        configs.append('component name={0}.affine type=BayesTdnnComponent '
                       'input-dim={1} output-dim={2} l2-regularize={3} max-change={4} '
                       'time-offsets={5} test-flag={6} use-exp-std={7} share-std-output-sampling={8} KL-scale={9} prior-std={10}'.format(
                           name, bottleneck_dim, output_dim, l2_regularize,
                           max_change, time_offsets2, test_flag, use_exp_std, share_std_output_sampling, KL_scale, prior_std))
        configs.append('component-node name={0}.affine component={0}.affine '
                       'input={0}.linear'.format(name))

        # The ReLU layer
        #configs.append('component name={0}.gpact type=TanhComponent dim={1} '
        #               'self-repair-scale={2}'.format(
        #                   name, output_dim, self_repair_scale))
        configs.append('component name={0}.gpact type=NaturalGradientGPActivationComponent input-dim={1} output-dim={2} '
                       ' max-change={3} test-mode={4} use-exp-std={5} share-std-output-sampling={6} KL-scale={7} prior-std={8}'.format(
                           name, output_dim, output_dim, max_change, gpact_test_mode, use_exp_std, share_std_output_sampling, gpact_KL_scale, prior_std))

        configs.append('component-node name={0}.gpact component={0}.gpact '
                       'input={0}.affine'.format(name))
#component name=tdnn1.gpact type=NaturalGradientGPActivationComponent input-dim=1536 output-dim=1536 test-mode=true use-exp-std=true share-std-output-sampling=true KL-scale=0 max-change=0.75
        # The BatchNorm layer
        configs.append('component name={0}.batchnorm type=BatchNormComponent '
                       'dim={1}'.format(name, output_dim))
        configs.append('component-node name={0}.batchnorm component={0}.batchnorm '
                       'input={0}.gpact'.format(name))
