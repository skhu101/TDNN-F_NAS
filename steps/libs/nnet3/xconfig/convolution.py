...
class XconfigConvDARTSLayer(XconfigLayerBase):
    def __init__(self, first_token, key_to_value, prev_names = None):
        for operation in first_token.split('-')[:-1]:
            assert operation in ['convdarts', 'renorm', 'batchnorm', 'relu',
                                 'noconv', 'dropout', 'so']
        XconfigLayerBase.__init__(self, first_token, key_to_value, prev_names)
    ...
    def set_default_configs(self):
        self.config = {'input':'[-1]',
                       ...
                       'rank-in':'', 'rank-out':'', 'num-minibatches-history':'',
                       'alpha-in':'', 'alpha-out':'', 'l2-regularize':'',
                       'dropout-proportion': 0.5, 'use-gumbel': 'false', 'update-alpha': 'true', 'uniform-sample': 'false', 'Temp-Proportion': 1.0}
    ...
    def output_name(self, auxiliary_output = None):
        ...
        assert len(operations) >= 1
        last_operation = operations[-1]
        assert last_operation in ['relu', 'convdarts', 'renorm', 'batchnorm', 'dropout', 'so']
        # we'll return something like 'layer1.batchnorm'.
        return '{0}.{1}'.format(self.name, last_operation)
        ...
        for operation in operations:
            if operation == 'convdarts':
                ...
                for opt_name in [
                        ...
                        'height-in','height-out', 'height-subsample-out',
                        'height-offsets', 'time-offsets', 'required-time-offsets',
                        'learning-rate-factor', 'l2-regularize', 'use-gumbel', 'update-alpha', 'uniform-sample', 'Temp-Proportion']:
                    value = self.config[opt_name]
                    if value != '':
                        a.append('{0}={1}'.format(opt_name, value))
                conv_opts = ' '.join(a)

                configs.append('component name={0}.conv type=TimeHeightConvolutionDARTSComponent '
                               '{1}'.format(name, conv_opts))
                configs.append('component-node name={0}.conv component={0}.conv '
                               'input={1}'.format(name, cur_descriptor))
                cur_num_filters = self.config['num-filters-out']
                ...
                configs.append('component name={0}.relu type=RectifiedLinearComponent '
                               ...
                               'self-repair-lower-threshold={4}'.format(
                                   ...
                                   self.config['self-repair-lower-threshold']))
                configs.append('component-node name={0}.relu component={0}.relu '
                               'input={1}'.format(name, cur_descriptor.replace('convdarts', 'conv')))
            elif operation == 'dropout':
                configs.append('component name={0}.dropout type=DropoutComponent '
                           'dim={1} dropout-proportion={2}'.format(
                               name, cur_num_filters * cur_height,
...
class XconfigConvDARTSTimeLayer(XconfigLayerBase):
    def __init__(self, first_token, key_to_value, prev_names = None):
        for operation in first_token.split('-')[:-1]:
            assert operation in ['convdartstime', 'renorm', 'batchnorm', 'relu',
                                 'noconv', 'dropout', 'so']
        XconfigLayerBase.__init__(self, first_token, key_to_value, prev_names)
    ...
    def output_name(self, auxiliary_output = None):
        ...
        assert len(operations) >= 1
        last_operation = operations[-1]
        assert last_operation in ['relu', 'convdartstime', 'renorm', 'batchnorm', 'dropout', 'so']
        # we'll return something like 'layer1.batchnorm'.
        return '{0}.{1}'.format(self.name, last_operation)
    ...
    def _generate_cnn_config(self):
        ...
        for operation in operations:
            if operation == 'convdartstime':
                ...
                configs.append('component name={0}.conv type=TimeHeightConvolutionDARTSTimeComponent '
                               '{1}'.format(name, conv_opts))
                configs.append('component-node name={0}.conv component={0}.conv '
                               'input={1}'.format(name, cur_descriptor))
                cur_num_filters = self.config['num-filters-out']
                ...
                configs.append('component name={0}.relu type=RectifiedLinearComponent '
                               ...
                               'self-repair-lower-threshold={4}'.format(
                                   ...
                                   self.config['self-repair-lower-threshold']))
                configs.append('component-node name={0}.relu component={0}.relu '
                               'input={1}'.format(name, cur_descriptor.replace('convdartstime', 'conv')))
            elif operation == 'dropout':
                configs.append('component name={0}.dropout type=DropoutComponent '
                           'dim={1} dropout-proportion={2}'.format(
                               name, cur_num_filters * cur_height,
