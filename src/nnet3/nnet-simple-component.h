...
class NormalRandComponent: public RandomComponent {
 public:
  void Init(int32 dim, int32 input_dim, bool rand_per_frame = false);
  NormalRandComponent(int32 dim, int32 input_dim, bool rand_per_frame = false) {
...
class BayesAffineComponent: public UpdatableComponent {
 public:
  ...
  // BayesAffineComponent(): orthonormal_constraint_(0.0), KL_scale_(0.0001), 
  //             test_mode_(false), add_weight_noise_(false), share_std_input_(false), share_std_output_(false),
  ...
  // The next constructor is used in converting from nnet1.
  BayesAffineComponent(const CuMatrixBase<BaseFloat> &linear_params,
          const CuMatrixBase<BaseFloat> &linear_prior_mean,
          const CuMatrixBase<BaseFloat> &linear_prior_std,
                  const CuVectorBase<BaseFloat> &bias_params,
                  BaseFloat learning_rate);
...
class GPActivationComponent: public UpdatableComponent {
 public:
  virtual int32 InputDim() const { return activation_params_.NumCols(); }
  virtual int32 OutputDim() const { return activation_params_.NumCols(); }
  // virtual int32 InputDim() const { return linear_params_mean_.NumCols(); }
  // virtual int32 OutputDim() const { return linear_params_mean_.NumRows(); }
  ...
  GPActivationComponent(): orthonormal_constraint_(0.0), KL_scale_(0.0001), 
              test_mode_(false), share_std_input_(false), share_std_output_(false),
              share_std_input_sampling_(false), share_std_output_sampling_(false),
              use_exp_std_(false), update_prior_(false) { } // use Init to really initialize.
  virtual std::string Type() const { return "GPActivationComponent"; }
  virtual int32 Properties() const {
    return kSimpleComponent|kUpdatableComponent|
        kBackpropNeedsInput|kBackpropAdds|kUsesMemo;
  }
  ...
  const CuVector<BaseFloat> &BiasParams() const { return bias_params_; }
  CuVector<BaseFloat> &BiasParams() { return bias_params_; }
  const CuMatrix<BaseFloat> &ActivationParams() const { return activation_params_; }
  CuMatrix<BaseFloat> &ActivationParams() { return activation_params_; }
  explicit GPActivationComponent(const GPActivationComponent &other);
  // The next constructor is used in converting from nnet1.
  GPActivationComponent(const CuMatrixBase<BaseFloat> &activation_params,
          const CuMatrixBase<BaseFloat> &activation_prior_mean,
          const CuMatrixBase<BaseFloat> &activation_prior_std,
                  const CuVectorBase<BaseFloat> &bias_params,
                  BaseFloat learning_rate);
 ...
 protected:
  ...
  const GPActivationComponent &operator = (const GPActivationComponent &other); // Disallow.
  CuMatrix<BaseFloat> activation_params_;
  CuMatrix<BaseFloat> activation_prior_mean_;
  CuMatrix<BaseFloat> activation_prior_std_;
  CuVector<BaseFloat> bias_params_;
  // see documentation at the top of this class for more information on the
  // following.
  BaseFloat orthonormal_constraint_;
...
class NaturalGradientGPActivationComponent: public GPActivationComponent {
 public:
  virtual std::string Type() const { return "NaturalGradientGPActivationComponent"; }
  virtual void Read(std::istream &is, bool binary);
  virtual void Write(std::ostream &os, bool binary) const;
  // this constructor does not really initialize, use InitFromConfig() or Read().
  NaturalGradientGPActivationComponent() { }
  void InitFromConfig(ConfigLine *cfl);
  virtual std::string Info() const;
  virtual Component* Copy() const;
  virtual void Scale(BaseFloat scale);
  ...
  // copy constructor
  explicit NaturalGradientGPActivationComponent(
 ...
 private:
  // disallow assignment operator.
  NaturalGradientGPActivationComponent &operator= (
      const NaturalGradientGPActivationComponent&);
...
//////////////////////////////////////////////////////////////////////////////////////////////////
/**  FlopsConstraintComponent calculates the flops constraint
 *
 *      Accepted values on its config-file line are:
 *              input-dim  The input dimension.  Required.
 *                      output-dim  The block dimension.  Required.  Must divide input-dim.
 *                              flops  The flops constraint on top of the probability component (SoftmaxComponent). 
 *                                      scale      A scaling factor on the output.  Defaults to 1.0.
 *                                       */
class FlopsConstraintComponent: public Component {
 public:
  explicit FlopsConstraintComponent(const FlopsConstraintComponent &other);
  FlopsConstraintComponent() { }
  virtual std::string Type() const { return "FlopsConstraintComponent"; }
  virtual int32 Properties() const {
    return kSimpleComponent|kPropagateAdds|kBackpropAdds|kBackpropNeedsInput;
  }
  virtual void InitFromConfig(ConfigLine *cfl);
  virtual int32 InputDim() const { return input_dim_; }
  virtual int32 OutputDim() const { return output_dim_; }
  ...
  virtual void Write(std::ostream &os, bool binary) const;
  virtual std::string Info() const;
  virtual Component* Copy() const { return new FlopsConstraintComponent(*this); }
  virtual void* Propagate(const ComponentPrecomputedIndexes *indexes,
                          const CuMatrixBase<BaseFloat> &in, 
                          CuMatrixBase<BaseFloat> *out) const;
  virtual void Backprop(const std::string &debug_info,
 ...
 private:
  ...
  int32 output_dim_;
  BaseFloat scale_;
  std::vector<int32> flops_;
  CuVector<BaseFloat> cuflops_;
  FlopsConstraintComponent &operator = (const FlopsConstraintComponent &other); // Disallow.
};

// OnehotFunctionComponent returns constant function of its input,
// i.e. its output does not depend on its input.  It is the same as
// an affine component with the linear term fixed at zero.
// It is optionally trainable, and optionally you can use natural
// gradient.  The input is required only because it's more convenient
// to make SimpleComponents [but see ConstantComponent, which requires
// no inputs].
class OnehotFunctionComponent: public UpdatableComponent {
 public:
  virtual int32 InputDim() const { return input_dim_; }
  virtual int32 OutputDim() const { return output_.Dim(); }
  ...
  OnehotFunctionComponent();

  OnehotFunctionComponent(const OnehotFunctionComponent &other);

  virtual std::string Type() const { return "OnehotFunctionComponent"; }
  virtual int32 Properties() const {
    return kSimpleComponent|
        (is_updatable_ ? kUpdatableComponent : 0) |
        (InputDim() == OutputDim() ? kPropagateInPlace: 0) |
 ...
 private:
  ...
  const OnehotFunctionComponent &operator
  = (const OnehotFunctionComponent &other); // Disallow.
};


class SoftmaxgradnormComponent: public NonlinearComponent {
 public:
  explicit SoftmaxgradnormComponent(const SoftmaxgradnormComponent &other):
      NonlinearComponent(other) { }
  SoftmaxgradnormComponent() { }
  virtual std::string Type() const { return "SoftmaxgradnormComponent"; }
  virtual int32 Properties() const {
    return kSimpleComponent|kPropagateInPlace|kBackpropInPlace|
        kBackpropNeedsOutput|kStoresStats;
  }
  ...
  virtual void StoreStats(const CuMatrixBase<BaseFloat> &in_value,
                          const CuMatrixBase<BaseFloat> &out_value,
                          void *memo);
  virtual Component* Copy() const { return new SoftmaxgradnormComponent(*this); }
 private:
  SoftmaxgradnormComponent &operator = (const SoftmaxgradnormComponent &other); // Disallow.
};


///////////////////////////////////////////////////////////////////////////////////////
//This component returns an one-hot vector by setting the value with largest value from 
//input vector to be 1 and others to be 0.
class ArgmaxOnehotFunctionComponent: public NonlinearComponent {
 public:
  explicit ArgmaxOnehotFunctionComponent(const ArgmaxOnehotFunctionComponent &other):
      NonlinearComponent(other) { }
  ArgmaxOnehotFunctionComponent() { }
  virtual std::string Type() const { return "ArgmaxOnehotFunctionComponent"; }
  virtual int32 Properties() const {
    return kSimpleComponent|kPropagateInPlace|kBackpropInPlace|
        kBackpropNeedsOutput|kStoresStats;
  }
  ...
  virtual void StoreStats(const CuMatrixBase<BaseFloat> &in_value,
                          const CuMatrixBase<BaseFloat> &out_value,
                          void *memo);
  virtual Component* Copy() const { return new ArgmaxOnehotFunctionComponent(*this); }
 private:
  ArgmaxOnehotFunctionComponent &operator = (const ArgmaxOnehotFunctionComponent &other); // Disallow.
};


///////////////////////////////////////////////////////////////////////////////////////
// This component randomly zeros temp_proportion of the input
// and the derivatives are backpropagated through the nonzero inputs.
// Typically this component used during training but not in test time.
// The idea is described under the name Dropout, in the paper
// "Dropout: A Simple Way to Prevent Neural Networks from Overfitting".
class GumbelSoftmaxComponent : public RandomComponent {
 public:
  void Init(int32 dim, BaseFloat temp_proportion = 1.0);

  GumbelSoftmaxComponent(int32 dim, BaseFloat dropout = 1.0) {
    Init(dim, dropout);
  }

  GumbelSoftmaxComponent(): dim_(0), temp_proportion_(1.0){ }
  ...
  void SetTempProportion(BaseFloat temp_proportion) {
    temp_proportion_ = temp_proportion;
  }

  BaseFloat TempProportion() const { return temp_proportion_; }
 private:
  int32 dim_;
  /// dropout-proportion is the proportion that is dropped out,
  /// e.g. if 0.1, we set 10% to zero value.
...
class SoftmaxFlopsComponent : public RandomComponent {
 public:
  void Init(int32 dim, BaseFloat scale = 1.0);

  SoftmaxFlopsComponent(int32 dim, BaseFloat scale = 1.0) {
    Init(dim, scale);
  }

  SoftmaxFlopsComponent(): dim_(0), scale_(1.0){ }

  SoftmaxFlopsComponent(const SoftmaxFlopsComponent &other);

  virtual int32 Properties() const {
    return kBackpropInPlace|kSimpleComponent|kBackpropNeedsInput|
        kBackpropNeedsOutput|kRandomComponent;
  }
  virtual std::string Type() const { return "SoftmaxFlopsComponent"; }
...
// The idea is described under the name Dropout, in the paper
// "Dropout: A Simple Way to Prevent Neural Networks from Overfitting".
class GumbelSoftmaxFlopsComponent : public RandomComponent {
 public:
  void Init(int32 dim, BaseFloat scale = 1.0, BaseFloat temp_proportion = 1.0);

  GumbelSoftmaxFlopsComponent(int32 dim, BaseFloat scale = 1.0, BaseFloat dropout = 1.0) {
    Init(dim, scale, dropout);
  }

  GumbelSoftmaxFlopsComponent(): dim_(0), scale_(1.0), temp_proportion_(1.0){ }

  GumbelSoftmaxFlopsComponent(const GumbelSoftmaxFlopsComponent &other);

  virtual int32 Properties() const {
    return kBackpropInPlace|kSimpleComponent|kBackpropNeedsInput|
        kBackpropNeedsOutput|kRandomComponent;
  }
  virtual std::string Type() const { return "GumbelSoftmaxFlopsComponent"; }
