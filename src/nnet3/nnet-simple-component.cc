  ...
  if (to_update_in) {
    ...
    if (to_update->is_updatable_) {
      ...
      if (to_update->use_natural_gradient_ && !to_update->is_gradient_) {
        ...
        to_update->output_.AddRowSumMat(5*to_update->learning_rate_,
                                        out_deriv);
      }
    }
    cout << "log_alpha " << output_ << endl;
  }
}
...
void SoftmaxComponent::Backprop(const std::string &debug_info,
  ...
  //CuVector<BaseFloat> prob_grad;
  //prob_grad.Resize(4);
  //prob_grad.SetZero();
  //prob_grad.AddRowSumMat(1.0, out_deriv);
  //cout << "prob_grad " << prob_grad << endl;
...
void BayesAffineComponent::Resize(int32 input_dim, int32 output_dim) {
  ...
  linear_params_.Resize(output_dim*2, input_dim);
  linear_prior_mean_.Resize(output_dim, input_dim);
  linear_prior_std_.Resize(output_dim, input_dim);
...
void BayesAffineComponent::Add(BaseFloat alpha, const Component &other_in) {
  ...
  linear_prior_std_.AddMat(alpha, other->linear_prior_std_);
  bias_params_.AddVec(alpha, other->bias_params_);
}
...
BayesAffineComponent::BayesAffineComponent(const BayesAffineComponent &component):
    ...
    // add_weight_noise_(component.add_weight_noise_),
...
BayesAffineComponent::BayesAffineComponent(const CuMatrixBase<BaseFloat> &linear_params_,
  ...
  //linear_params_.Resize(linear_params_mean_.NumRows(), linear_params_mean_.NumCols());
  SetUnderlyingLearningRate(learning_rate);
  KALDI_ASSERT(linear_params_.NumRows()/2 == bias_params.Dim()&&
               bias_params.Dim() != 0);
}
...
void BayesAffineComponent::SetParams(const CuVectorBase<BaseFloat> &bias,
  ...
  linear_prior_mean_ = linear_prior_mean;
  linear_prior_std_ = linear_prior_std;
  KALDI_ASSERT(bias_params_.Dim() == linear_params_.NumRows()/2);
  //rand_mat_.Resize(linear_params_mean_.NumRows(), linear_params_mean_.NumCols());
  //linear_params_.Resize(linear_params_mean_.NumRows(), linear_params_mean_.NumCols());
}
...
std::string BayesAffineComponent::Info() const {
  ...
  // stream << ", add-weight-noise_=" << add_weight_noise_; 
...
BaseFloat BayesAffineComponent::DotProduct(const UpdatableComponent &other_in) const {
  ...
  return TraceMatMat(linear_params_, other->linear_params_, kTrans)
      ...
      + TraceMatMat(linear_prior_std_, other->linear_prior_std_, kTrans)
      + VecVec(bias_params_, other->bias_params_);
}
...
void BayesAffineComponent::Init(int32 input_dim, int32 output_dim,
  ...
  linear_params_.Resize(output_dim*2, input_dim);
  CuMatrix<BaseFloat> linear_params_mean_(output_dim, input_dim);
  CuMatrix<BaseFloat> linear_params_std_(output_dim, input_dim);
  // linear_prior_mean_.Resize(output_dim, input_dim);
  int32 input_dim_re = input_dim;
  int32 output_dim_re = output_dim;
  // if (share_std_input_) {
  ...
  //   output_dim_re = 1;
  // }
  linear_prior_mean_.Resize(output_dim_re, input_dim_re);
  linear_prior_std_.Resize(output_dim_re, input_dim_re);
  bias_params_.Resize(output_dim);
  ...
  linear_params_.Range(linear_params_.NumRows()/2, linear_params_.NumRows()/2, 0, linear_params_.NumCols()).CopyFromMat(linear_params_std_);
...
void BayesAffineComponent::Init(std::string matrix_filename) {
  ...
  linear_params_.Range(0, linear_params_.NumRows()/2, 0, linear_params_.NumCols()).CopyFromMat(mat.Range(0, output_dim, 0, input_dim));
  linear_params_.Range(linear_params_.NumRows()/2, linear_params_.NumRows()/2, 0, linear_params_.NumCols()).CopyFromMat(mat.Range(0, output_dim_re, input_dim, input_dim_re));  
  linear_prior_mean_.CopyFromMat(mat.Range(0, output_dim/2, 2*input_dim, input_dim));
  linear_prior_std_.CopyFromMat(mat.Range(0, output_dim_re/2, 3*input_dim, input_dim_re));
...
void BayesAffineComponent::InitFromConfig(ConfigLine *cfl) {
  ...
  InitLearningRatesFromConfig(cfl);
  cfl->GetValue("test-mode", &test_mode_);
  // cfl->GetValue("add-weight-noise", &add_weight_noise_);
...
void* BayesAffineComponent::Propagate(const ComponentPrecomputedIndexes *indexes,
  ...
  CuSubMatrix<BaseFloat> linear_params_std_(linear_params_, 
                                        linear_params_.NumRows()/2, linear_params_.NumRows()/2,
                                        0, linear_params_.NumCols());
  int32 input_dim_re = dim_input;
  int32 output_dim_re = dim_output;
  if (share_std_input_) {
    input_dim_re = 1;
  }
  if (share_std_output_) {
  ...
  CuSubMatrix<BaseFloat> linear_params_std_tie_row_(linear_params_, 
                                        linear_params_.NumRows()/2, 1,
                                        0, linear_params_.NumCols());

  CuMatrix<BaseFloat> linear_params(dim_output, dim_input);

  // if (add_weight_noise_){
  //   linear_params.AddMat(1.0, rand_mat_); // std*rand
  // } else{
  //   linear_params.AddMatBlocks(1.0, linear_params_std_tie_row_, kNoTrans); // std
  //   if (use_exp_std_) {
  ...
  // linear_params.AddMatBlocks(1.0, linear_params_std_, kNoTrans); // std

  linear_params.AddMat(1.0, linear_params_mean_); // std*rand + mean
  
  out->AddMatMat(1.0, in, kNoTrans, linear_params, kTrans, 1.0);
  // KALDI_LOG << "Propagate linear_params_mean_ " << linear_params_mean_(0,0);  
  // KALDI_LOG << "Propagate linear_params_std_ " << linear_params_std_(0,0);   
  // KALDI_LOG << "Propagate rand_mat_ " << rand_mat_(0,0);  
  // KALDI_LOG << "Propagate linear_params " << linear_params(0,0);  
  ...
  (rand_memo->ColRange(3*dim_input,dim_input)).CopyFromMat(linear_params_std_);
  (rand_memo->ColRange(4*dim_input,dim_input)).CopyFromMat(linear_prior_mean_);
  (rand_memo->ColRange(5*dim_input,dim_input)).CopyFromMat(linear_prior_std_);
  return rand_memo;
}
...
void BayesAffineComponent::Backprop(const std::string &debug_info,
  ...
  CuMatrix<BaseFloat> linear_params_std_(dim_output, dim_input);
  CuMatrix<BaseFloat> linear_prior_mean_(dim_output, dim_input);
  CuMatrix<BaseFloat> linear_prior_std_(dim_output, dim_input);
  ...
  linear_params_std_.CopyFromMat(rand_memo->ColRange(3*dim_input,dim_input));
  linear_prior_mean_.CopyFromMat(rand_memo->ColRange(4*dim_input,dim_input));
  linear_prior_std_.CopyFromMat(rand_memo->ColRange(5*dim_input,dim_input));
  ...
  // in_deriv, in case of infinities.
  if (in_deriv)
    in_deriv->AddMatMat(1.0, out_deriv, kNoTrans, linear_params, kNoTrans,
                        1.0);
  ...
  if (to_update != NULL) {
    ...
    if (to_update->is_gradient_)
      ...
      to_update->Update(debug_info, in_value, out_deriv, rand_mat_, linear_params_mean_, linear_params_std_, linear_prior_mean_, linear_prior_std_);  // by child classes.
  }
}
...
void BayesAffineComponent::Read(std::istream &is, bool binary) {
  ...
  ExpectToken(is, binary, "<TestMode>");
  ReadBasicType(is, binary, &test_mode_);
  // ExpectToken(is, binary, "<AddWeightNoise>");
  // ReadBasicType(is, binary, &add_weight_noise_);  
...
int32 BayesAffineComponent::NumParameters() const {
  int32 in_dim_std = linear_params_.NumCols(), out_dim_std = linear_params_.NumRows()/2;
...
void BayesAffineComponent::Vectorize(VectorBase<BaseFloat> *params) const {
  ...
  int32 in_dim_std = linear_params_.NumCols(), 
  out_dim_std = linear_params_.NumRows()/2, 
  pose_current;
...
void BayesAffineComponent::UnVectorize(const VectorBase<BaseFloat> &params) {
  ...
  out_dim_std = linear_params_.NumRows()/2, 
  pose_current;
  CuMatrix<BaseFloat> linear_params_mean_(out_dim_std, in_dim_std);
  CuMatrix<BaseFloat> linear_params_std_(out_dim_std, in_dim_std);
...
NaturalGradientBayesAffineComponent::NaturalGradientBayesAffineComponent(
    const CuMatrixBase<BaseFloat> &linear_params_,
    const CuMatrixBase<BaseFloat> &linear_prior_mean,
    const CuMatrixBase<BaseFloat> &linear_prior_std,
        const CuVectorBase<BaseFloat> &bias_params):
    BayesAffineComponent(linear_params_, linear_prior_mean, linear_prior_std, bias_params, 0.001) {
  KALDI_ASSERT(bias_params.Dim() == linear_params_.NumRows()/2 &&
               bias_params.Dim() != 0);
...
void NaturalGradientBayesAffineComponent::InitFromConfig(ConfigLine *cfl) {
  ...
  cfl->GetValue("share-std-output-sampling", &share_std_output_sampling_);
  cfl->GetValue("use-exp-std", &use_exp_std_);
  cfl->GetValue("update-prior", &update_prior_);
  cfl->GetValue("KL-scale", &KL_scale_);
  ...
  // }
    ...
    linear_params_std_.Scale(param_std_stddev);
    if (!use_exp_std_) {
      linear_params_std_.Add(-2.0); // init around -1
    } else {
      linear_params_std_.Add(prior_std); // init around -1
    }
...
void NaturalGradientBayesAffineComponent::Write(std::ostream &os,
  ...
  WriteToken(os, binary, "<TestMode>");
  WriteBasicType(os, binary, test_mode_);
  // WriteToken(os, binary, "<AddWeightNoise>");
  // WriteBasicType(os, binary, add_weight_noise_);  
  WriteToken(os, binary, "<ShareStdInput>");
  WriteBasicType(os, binary, share_std_input_);
  WriteToken(os, binary, "<ShareStdOutput>");
  WriteBasicType(os, binary, share_std_output_);
...
void NaturalGradientBayesAffineComponent::Update(
    ...
    const CuMatrixBase<BaseFloat> &linear_params_std_tmp_,
    const CuMatrixBase<BaseFloat> &linear_prior_mean_tmp_,
    const CuMatrixBase<BaseFloat> &linear_prior_std_tmp_) {
  CuMatrix<BaseFloat> in_value_temp;
  ...
  linear_deriv.MulElements(rand_mat);
  // CuMatrix<BaseFloat> linear_params_std_expand_(dim_output, dim_input);
  // linear_params_std_expand_.AddMatBlocks(1.0, linear_params_std_, kNoTrans);

  CuSubMatrix<BaseFloat> linear_params_std_tie_row_tmp_(linear_params_std_tmp_, 
                                        0, 1,
                                        0, linear_params_std_tmp_.NumCols());
  CuSubMatrix<BaseFloat> linear_prior_std_tie_row_tmp_(linear_prior_std_tmp_, 
                                        0, 1,
                                        0, linear_prior_std_tmp_.NumCols());

  //CuMatrix<BaseFloat> linear_prior_std_tie_row_tmp_(1, linear_prior_std_tmp_.NumCols());
  //linear_prior_std_tie_row_tmp_.Set(-3.0);

  if (use_exp_std_) {
    CuMatrix<BaseFloat> linear_params_std_exp(dim_output, dim_input);
    linear_params_std_exp.AddMatBlocks(1.0, linear_params_std_tie_row_tmp_, kNoTrans); // std
    // linear_params_std_exp.CopyFromMat(linear_params_std_tmp_);
    linear_params_std_exp.ApplyCeiling(46.05);
    linear_params_std_exp.ApplyExp();
    linear_deriv.MulElements(linear_params_std_exp);
  }
  ...
  // float value1=linear_params_std_(0,0);
  // KALDI_LOG << "value1: " << value1;
  ...
  if (KL_scale_ != 0) {
    ...
    CuMatrix<BaseFloat> linear_params_std_resize(dim_output, dim_input);
    CuMatrix<BaseFloat> linear_prior_std_resize(dim_output, dim_input);
    linear_params_std_resize.AddMatBlocks(1.0, linear_params_std_tie_row_tmp_, kNoTrans);
    linear_prior_std_resize.AddMatBlocks(1.0, linear_prior_std_tie_row_tmp_, kNoTrans);
  ...
  linear_params_.Range(0, linear_params_.NumRows()/2, 0, linear_params_.NumCols()).CopyFromMat(linear_params_mean_);
  linear_params_.Range(linear_params_.NumRows()/2, 1, 0, linear_params_.NumCols()).CopyFromMat(linear_params_std_); 
}
...
void NaturalGradientBayesAffineComponent::Add(BaseFloat alpha, const Component &other_in) {
  ...
  linear_params_.AddMat(alpha, other->linear_params_);
  linear_prior_mean_.AddMat(alpha, other->linear_prior_mean_);
  linear_prior_std_.AddMat(alpha, other->linear_prior_mean_);
  bias_params_.AddVec(alpha, other->bias_params_);
}
...
void GPActivationComponent::Scale(BaseFloat scale) {
  if (scale == 0.0) {
    // If scale == 0.0 we call SetZero() which will get rid of NaN's and inf's.
  activation_params_.SetZero();
  activation_prior_mean_.SetZero();
  activation_prior_std_.SetZero();
    bias_params_.SetZero();
  } else {
  activation_params_.Scale(scale);
  activation_prior_mean_.Scale(scale);
  activation_prior_std_.Scale(scale);
    bias_params_.Scale(scale);
  }
}

void GPActivationComponent::Resize(int32 input_dim, int32 output_dim) {
  KALDI_ASSERT(input_dim > 0 && output_dim > 0);
  bias_params_.Resize(output_dim);


  // activation_params_mean_.Resize(output_dim, input_dim);
  activation_params_.Resize(output_dim*2, input_dim);
  activation_prior_mean_.Resize(output_dim, input_dim);
  activation_prior_std_.Resize(output_dim, input_dim);
  //rand_mat_.Resize(output_dim, input_dim);
  //activation_params_.Resize(output_dim, input_dim);
  ...
  //   output_dim_re = 1;
  // }
  // activation_params_std_.Resize(output_dim_re, input_dim_re);
  // activation_prior_std_.Resize(output_dim_re, input_dim_re);

}

void GPActivationComponent::Add(BaseFloat alpha, const Component &other_in) {
  const GPActivationComponent *other =
      dynamic_cast<const GPActivationComponent*>(&other_in);
  KALDI_ASSERT(other != NULL);
  activation_params_.AddMat(alpha, other->activation_params_);
  activation_prior_mean_.AddMat(alpha, other->activation_prior_mean_);
  activation_prior_std_.AddMat(alpha, other->activation_prior_std_);
  bias_params_.AddVec(alpha, other->bias_params_);
}

GPActivationComponent::GPActivationComponent(const GPActivationComponent &component):
    UpdatableComponent(component),
    activation_params_(component.activation_params_),
    activation_prior_mean_(component.activation_prior_mean_),
    activation_prior_std_(component.activation_prior_std_),
    bias_params_(component.bias_params_),
    test_mode_(component.test_mode_),
    share_std_input_(component.share_std_input_),
    share_std_output_(component.share_std_output_),
    ...
    KL_scale_(component.KL_scale_),
    orthonormal_constraint_(component.orthonormal_constraint_) { 
    //rand_mat_.Resize(activation_params_mean_.NumRows(), activation_params_mean_.NumCols());
    //activation_params_.Resize(activation_params_mean_.NumRows(), activation_params_mean_.NumCols());
    }

GPActivationComponent::GPActivationComponent(const CuMatrixBase<BaseFloat> &activation_params_,
          const CuMatrixBase<BaseFloat> &activation_prior_mean,
          const CuMatrixBase<BaseFloat> &activation_prior_std,
                  const CuVectorBase<BaseFloat> &bias_params,
                                 BaseFloat learning_rate):
  activation_params_(activation_params_),
  activation_prior_mean_(activation_prior_mean),
  activation_prior_std_(activation_prior_std),
  bias_params_(bias_params),
  test_mode_(false),
  share_std_input_(false),
  share_std_output_(false),
  ...
  //activation_params_.Resize(activation_params_mean_.NumRows(), activation_params_mean_.NumCols());
  SetUnderlyingLearningRate(learning_rate);
  KALDI_ASSERT(activation_params_.NumRows()/2 == bias_params.Dim()&&
               bias_params.Dim() != 0);
}

void GPActivationComponent::SetParams(const CuVectorBase<BaseFloat> &bias,
                         const CuMatrixBase<BaseFloat> &activation,
             const CuMatrixBase<BaseFloat> &activation_prior_mean,
             const CuMatrixBase<BaseFloat> &activation_prior_std) {
  bias_params_ = bias;
  activation_params_ = activation;
  activation_prior_mean_ = activation_prior_mean;
  activation_prior_std_ = activation_prior_std;
  KALDI_ASSERT(bias_params_.Dim() == activation_params_.NumRows()/2);
  //rand_mat_.Resize(activation_params_mean_.NumRows(), activation_params_mean_.NumCols());
  //activation_params_.Resize(activation_params_mean_.NumRows(), activation_params_mean_.NumCols());
}

void GPActivationComponent::PerturbParams(BaseFloat stddev) {
  CuMatrix<BaseFloat> temp_activation_params(activation_params_);
  temp_activation_params.SetRandn();
  activation_params_.AddMat(stddev, temp_activation_params);
...
std::string GPActivationComponent::Info() const {
  std::ostringstream stream;
  stream << UpdatableComponent::Info();
  if (orthonormal_constraint_ != 0.0)
    stream << ", orthonormal-constraint=" << orthonormal_constraint_;
  ...
  stream << ", update-prior=" << update_prior_;
  stream << ", KL-scale=" << KL_scale_;
  PrintParameterStats(stream, "activation-params", activation_params_,
                      false, // include_mean
                      true, // include_row_norms
                      true, // include_column_norms
                      GetVerboseLevel() >= 2); // include_singular_values
  // PrintParameterStats(stream, "activation-params-std", activation_params_std_,
  //                     false, // include_mean
  //                     true, // include_row_norms
  //                     true, // include_column_norms
  //                     GetVerboseLevel() >= 2); // include_singular_values
  PrintParameterStats(stream, "activation-prior-mean", activation_prior_mean_,
                      false, // include_mean
                      true, // include_row_norms
                      true, // include_column_norms
                      GetVerboseLevel() >= 2); // include_singular_values
  PrintParameterStats(stream, "activation-prior-std", activation_prior_std_,
                      false, // include_mean
                      true, // include_row_norms
                      true, // include_column_norms
                      GetVerboseLevel() >= 2); // include_singular_values
...
Component* GPActivationComponent::Copy() const {
  GPActivationComponent *ans = new GPActivationComponent(*this);
  return ans;
}

BaseFloat GPActivationComponent::DotProduct(const UpdatableComponent &other_in) const {
  const GPActivationComponent *other =
      dynamic_cast<const GPActivationComponent*>(&other_in);
  return TraceMatMat(activation_params_, other->activation_params_, kTrans)
      + TraceMatMat(activation_prior_mean_, other->activation_prior_mean_, kTrans)
      + TraceMatMat(activation_prior_std_, other->activation_prior_std_, kTrans)
      + VecVec(bias_params_, other->bias_params_);
}

void GPActivationComponent::Init(int32 input_dim, int32 output_dim,
                           BaseFloat param_mean_stddev, BaseFloat param_std_stddev, BaseFloat bias_stddev, BaseFloat prior_mean, BaseFloat prior_std) {
  KALDI_ASSERT(output_dim > 0 && input_dim > 0 && param_mean_stddev >= 0.0 && param_std_stddev >= 0.0);


  activation_params_.Resize(output_dim*2, input_dim);
  CuMatrix<BaseFloat> activation_params_mean_(output_dim, input_dim);
  CuMatrix<BaseFloat> activation_params_std_(output_dim, input_dim);
  // activation_prior_mean_.Resize(output_dim, input_dim);
  int32 input_dim_re = input_dim;
  int32 output_dim_re = output_dim;
  // if (share_std_input_) {
  ...
  //   output_dim_re = 1;
  // }
  activation_prior_mean_.Resize(output_dim_re, input_dim_re);
  activation_prior_std_.Resize(output_dim_re, input_dim_re);
  bias_params_.Resize(output_dim);
  
  activation_params_mean_.SetRandn(); // sets to random normally distributed noise.
  activation_params_mean_.Scale(param_mean_stddev);
  
  activation_params_std_.SetRandn(); // sets to random normally distributed noise.
  activation_params_std_.Scale(param_std_stddev);
  if (!use_exp_std_) {
    activation_params_std_.Add(1.0); // init around 1
  }
  
  activation_prior_mean_.Set(prior_mean);
  activation_prior_std_.Set(prior_std);
  activation_params_std_.Add(prior_std); // init around 1
  
  bias_params_.SetRandn();
  bias_params_.Scale(bias_stddev);

  activation_params_.Range(0, activation_params_.NumRows()/2, 0, activation_params_.NumCols()).CopyFromMat(activation_params_mean_);

  activation_params_.Range(activation_params_.NumRows()/2, activation_params_.NumRows()/2, 0, activation_params_.NumCols()).CopyFromMat(activation_params_std_);
  
  //rand_mat_.Resize(activation_params_mean_.NumRows(), activation_params_mean_.NumCols());
  //activation_params_.Resize(activation_params_mean_.NumRows(), activation_params_mean_.NumCols());
}

void GPActivationComponent::Init(std::string matrix_filename) {
  CuMatrix<BaseFloat> mat;
  ReadKaldiObject(matrix_filename, &mat); // will abort on failure.
  KALDI_ASSERT(mat.NumCols() >= 2);
  int32 output_dim = mat.NumRows();
  ...
  // activation_params_mean_.CopyFromMat(mat.Range(0, output_dim, 0, input_dim));
  // activation_params_std_.CopyFromMat(mat.Range(0, output_dim_re, input_dim, input_dim_re));
  activation_params_.Range(0, activation_params_.NumRows()/2, 0, activation_params_.NumCols()).CopyFromMat(mat.Range(0, output_dim, 0, input_dim));
  activation_params_.Range(activation_params_.NumRows()/2, activation_params_.NumRows()/2, 0, activation_params_.NumCols()).CopyFromMat(mat.Range(0, output_dim_re, input_dim, input_dim_re));  
  activation_prior_mean_.CopyFromMat(mat.Range(0, output_dim/2, 2*input_dim, input_dim));
  activation_prior_std_.CopyFromMat(mat.Range(0, output_dim_re/2, 3*input_dim, input_dim_re));
  bias_params_.CopyColFromMat(mat, 4*input_dim);
...
void GPActivationComponent::InitFromConfig(ConfigLine *cfl) {
  bool ok = true;
  std::string matrix_filename;
  int32 input_dim = -1, output_dim = -1;
  test_mode_ = false;
...
void* GPActivationComponent::Propagate(const ComponentPrecomputedIndexes *indexes,
                                const CuMatrixBase<BaseFloat> &in,
                                 CuMatrixBase<BaseFloat> *out) const {
  ...
  int32 dim_input = activation_params_.NumCols();
  int32 dim_output = activation_params_.NumRows()/2;
  //CuMatrix<BaseFloat> rand_mat(dim_output, dim_input);
  ...
  CuSubMatrix<BaseFloat> activation_params_mean_(activation_params_,
                                          0, activation_params_.NumRows()/2,
                                          0, activation_params_.NumCols());

  CuSubMatrix<BaseFloat> activation_params_std_(activation_params_, 
                                        activation_params_.NumRows()/2, activation_params_.NumRows()/2,
                                        0, activation_params_.NumCols());
  ...
  CuMatrix<BaseFloat> activation_params(dim_output, dim_input);


  activation_params.AddMatBlocks(1.0, activation_params_std_, kNoTrans); // std

  if (use_exp_std_) {
    activation_params.ApplyCeiling(46.05); // avoid inf
    activation_params.ApplyExp(); // std = exp()
  }

  activation_params.MulElements(rand_mat_); // std*rand
  activation_params.AddMat(1.0, activation_params_mean_); // std*rand + mean

  //normalization coefficients 
  activation_params.Transpose();
  activation_params.ApplySoftMaxPerRow(activation_params);
  activation_params.ApplyFloor(1.0e-20);
  activation_params.Transpose();

  CuSubMatrix<BaseFloat> sigmoid_params(activation_params, 
                                        0, 1,
                                        0, activation_params.NumCols());  

  CuSubMatrix<BaseFloat> relu_params(activation_params, 
                                        1, 1,
                                        0, activation_params.NumCols());  

  CuSubMatrix<BaseFloat> tanh_params(activation_params, 
                                        2, 1,
                                        0, activation_params.NumCols());  

  CuMatrix<BaseFloat> sigmoid_output(in.NumRows(), in.NumCols());  
  CuMatrix<BaseFloat> relu_output(in.NumRows(), in.NumCols()); 
  CuMatrix<BaseFloat> tanh_output(in.NumRows(), in.NumCols());

  CuMatrix<BaseFloat> sigmoid_params_expand(in.NumRows(), in.NumCols());  
  CuMatrix<BaseFloat> relu_params_expand(in.NumRows(), in.NumCols()); 
  CuMatrix<BaseFloat> tanh_params_expand(in.NumRows(), in.NumCols()); 

  sigmoid_params_expand.AddMatBlocks(1.0, sigmoid_params, kNoTrans);
  relu_params_expand.AddMatBlocks(1.0, relu_params, kNoTrans);
  tanh_params_expand.AddMatBlocks(1.0, tanh_params, kNoTrans);


  sigmoid_output.Sigmoid(in); 
  relu_output.CopyFromMat(in);
  relu_output.ApplyFloor(0.0);
  tanh_output.Tanh(in);

  //original part 
  sigmoid_output.MulElements(sigmoid_params_expand);
  relu_output.MulElements(relu_params_expand);
  tanh_output.MulElements(tanh_params_expand); 

  out->CopyFromMat(relu_output);
  out->AddMat(1.0, sigmoid_output);
  out->AddMat(1.0, tanh_output);
  //

  // add part
  // out->CopyFromMat(sigmoid_output);
  //add part

  // out->AddMatMat(1.0, in, kNoTrans, activation_params, kTrans, 1.0);

  CuMatrix<BaseFloat> *rand_memo = new CuMatrix<BaseFloat>(dim_output, 6*dim_input);
  (rand_memo->ColRange(0,dim_input)).CopyFromMat(rand_mat_);
  (rand_memo->ColRange(dim_input,dim_input)).CopyFromMat(activation_params);
  (rand_memo->ColRange(2*dim_input,dim_input)).CopyFromMat(activation_params_mean_);
  (rand_memo->ColRange(3*dim_input,dim_input)).CopyFromMat(activation_params_std_);
  (rand_memo->ColRange(4*dim_input,dim_input)).CopyFromMat(activation_prior_mean_);
  (rand_memo->ColRange(5*dim_input,dim_input)).CopyFromMat(activation_prior_std_);
  return rand_memo;
}

void GPActivationComponent::UpdateSimple(const CuMatrixBase<BaseFloat> &in_value,
                                   const CuMatrixBase<BaseFloat> &out_deriv,
                   const CuMatrixBase<BaseFloat> &rand_mat) {
  bias_params_.AddRowSumMat(learning_rate_, out_deriv, 1.0);
  ...
  int32 dim_input = activation_params_.NumCols();
  int32 dim_output = activation_params_.NumRows()/2;
  CuMatrix<BaseFloat> activation_deriv(dim_output, dim_input);
  CuSubMatrix<BaseFloat> activation_params_mean_(activation_params_,
                                          0, activation_params_.NumRows()/2,
                                          0, activation_params_.NumCols());
  CuSubMatrix<BaseFloat> activation_params_std_(activation_params_, 
  ...
  activation_deriv.AddMatMat(1.0, out_deriv, kTrans,
                           in_value, kNoTrans, 0.0);
  activation_params_mean_.AddMat(learning_rate_, activation_deriv);
  activation_deriv.MulElements(rand_mat);
  if (use_exp_std_) {
    CuMatrix<BaseFloat> activation_params_std_exp(activation_params_std_);
    activation_params_std_exp.ApplyCeiling(46.05);
    activation_params_std_exp.ApplyExp();
    activation_deriv.MulElements(activation_params_std_exp);
  }
  activation_params_std_.AddMatBlocks(learning_rate_, activation_deriv, kNoTrans);
  ...
  if (KL_scale_ != 0) {
    ...
    CuMatrix<BaseFloat> activation_params_std_resize(dim_output, dim_input);
    CuMatrix<BaseFloat> activation_prior_std_resize(dim_output, dim_input);
    activation_params_std_resize.AddMatBlocks(1.0, activation_params_std_, kNoTrans);
    activation_prior_std_resize.AddMatBlocks(1.0, activation_prior_std_, kNoTrans);
    
    if (use_exp_std_) {
      activation_params_std_resize.ApplyCeiling(46.05); // avoid inf
      activation_params_std_resize.ApplyExp(); // std = exp()
      activation_prior_std_resize.ApplyCeiling(46.05); // avoid inf
      activation_prior_std_resize.ApplyExp(); // std = exp()
    }
  
    inv_prior_std.CopyFromMat(activation_prior_std_resize);
    inv_prior_std.InvertElements(); // 1/prior_std
  
    rate.CopyFromMat(activation_params_std_resize);
    rate.MulElements(inv_prior_std); // post_std/prior_std
    CuMatrix<BaseFloat> rate_power_2(rate);
    rate_power_2.MulElements(rate); // post_std^2/prior_std^2
  
    diff.CopyFromMat(activation_params_mean_);
    diff.AddMat(-1.0, activation_prior_mean_); // post_mean - prior_mean
    diff.MulElements(inv_prior_std); // (post_mean - prior_mean)/prior_std
    ...
    post_std_deriv.MulElements(inv_prior_std); // post_std/prior_std^2
    CuMatrix<BaseFloat> inv_post_std(dim_output, dim_input);
    inv_post_std.CopyFromMat(activation_params_std_resize);
    inv_post_std.InvertElements(); // 1/post_std
    post_std_deriv.AddMat(-1.0, inv_post_std); // post_std/prior_std^2 - 1/post_std
    if (use_exp_std_) {
      post_std_deriv.MulElements(activation_params_std_resize);
    }
    
    BaseFloat learning_rate_KL_ = -1.0*learning_rate_*num_frame*KL_scale_;
    activation_params_mean_.AddMat(learning_rate_KL_, post_mean_deriv);
    activation_params_std_.AddMatBlocks(learning_rate_KL_, post_std_deriv, kNoTrans);
    ...
    if (update_prior_) {
      ...
      activation_prior_mean_.AddMat(learning_rate_KL_, prior_mean_deriv);
      activation_prior_std_.AddMatBlocks(learning_rate_KL_, prior_std_deriv, kNoTrans);
    }
  activation_params_.Range(0, activation_params_.NumRows()/2, 0, activation_params_.NumCols()).CopyFromMat(activation_params_mean_);
  activation_params_.Range(activation_params_.NumRows()/2, activation_params_.NumRows()/2, 0, activation_params_.NumCols()).CopyFromMat(activation_params_std_);
  }
...
void GPActivationComponent::Backprop(const std::string &debug_info,
                               const ComponentPrecomputedIndexes *indexes,
                               const CuMatrixBase<BaseFloat> &in_value,
                               const CuMatrixBase<BaseFloat> &, // out_value
                               const CuMatrixBase<BaseFloat> &out_deriv,
                               ...
                               Component *to_update_in,
                               CuMatrixBase<BaseFloat> *in_deriv) const {
  GPActivationComponent *to_update = dynamic_cast<GPActivationComponent*>(to_update_in);
  CuMatrix<BaseFloat> *rand_memo = reinterpret_cast<CuMatrix<BaseFloat>*>(memo);
  int32 dim_input = activation_params_.NumCols();
  int32 dim_output = activation_params_.NumRows()/2;
  CuMatrix<BaseFloat> rand_mat_(dim_output, dim_input);
  CuMatrix<BaseFloat> activation_params(dim_output, dim_input);
  CuMatrix<BaseFloat> activation_params_mean_(dim_output, dim_input);
  CuMatrix<BaseFloat> activation_params_std_(dim_output, dim_input);
  CuMatrix<BaseFloat> activation_prior_mean_(dim_output, dim_input);
  CuMatrix<BaseFloat> activation_prior_std_(dim_output, dim_input);

  rand_mat_.CopyFromMat(rand_memo->ColRange(0,dim_input));
  activation_params.CopyFromMat(rand_memo->ColRange(dim_input,dim_input));
  activation_params_mean_.CopyFromMat(rand_memo->ColRange(2*dim_input,dim_input));
  activation_params_std_.CopyFromMat(rand_memo->ColRange(3*dim_input,dim_input));
  activation_prior_mean_.CopyFromMat(rand_memo->ColRange(4*dim_input,dim_input));
  activation_prior_std_.CopyFromMat(rand_memo->ColRange(5*dim_input,dim_input));
  ...
  if (in_deriv){
    // //sigmoid
    CuMatrix<BaseFloat> sigmoid_out_value(in_value.NumRows(), in_value.NumCols());
    CuMatrix<BaseFloat> sigmoid_in_deriv(in_value.NumRows(), in_value.NumCols());
    CuMatrix<BaseFloat> sigmoid_out_deriv(out_deriv);
    CuSubVector<BaseFloat> sigmoid_scale_(activation_params, 0);

    sigmoid_out_value.Sigmoid(in_value);
    sigmoid_out_deriv.MulColsVec(sigmoid_scale_);

    sigmoid_in_deriv.DiffSigmoid(sigmoid_out_value, sigmoid_out_deriv);

    BaseFloat sigmoid_repair_scale_ = 0.00001;
    BaseFloat sigmoid_repair_probability = 0.5;
    BaseFloat sigmoid_lower_threshold = 0.05;
    
    if (RandUniform() <= sigmoid_repair_probability) {
      CuVector<BaseFloat> sigmoid_deriv_sum_(dim_input);
      sigmoid_deriv_sum_.AddRowSumMat(1.0, sigmoid_in_deriv, 1.0);

      CuMatrix<BaseFloat> sigmoid_thresholds(1, dim_input);
      CuSubVector<BaseFloat> sigmoid_thresholds_vec(sigmoid_thresholds, 0);
      sigmoid_thresholds_vec.AddVec(-1.0, sigmoid_deriv_sum_);
      sigmoid_thresholds_vec.Add(sigmoid_lower_threshold);
      sigmoid_thresholds.ApplyHeaviside();

      sigmoid_in_deriv.AddMatDiagVec(-2.0 * sigmoid_repair_scale_ / sigmoid_repair_probability,
                            sigmoid_out_value, kNoTrans, sigmoid_thresholds_vec);
      sigmoid_in_deriv.AddVecToRows(sigmoid_repair_scale_ / sigmoid_repair_probability,
                           sigmoid_thresholds_vec);
    }


    //ParametricRelu
    CuMatrix<BaseFloat> relu_out_value(in_value.NumRows(), in_value.NumCols());
    CuMatrix<BaseFloat> relu_in_deriv(in_value.NumRows(), in_value.NumCols());
    CuMatrix<BaseFloat> relu_out_deriv(out_deriv);
    CuSubVector<BaseFloat> relu_scale_(activation_params, 1);

    relu_out_value.CopyFromMat(in_value);
    relu_out_value.ApplyFloor(0.0);  
    relu_out_deriv.MulColsVec(relu_scale_);

    relu_in_deriv.Heaviside(relu_out_value);
    relu_in_deriv.MulElements(relu_out_deriv);
    BaseFloat relu_repair_probability = 0.5;

    if (RandUniform() <= relu_repair_probability) {
      BaseFloat relu_self_repair_scale_ = 0.00001;
      BaseFloat relu_lower_threshold = 0.05;
      BaseFloat relu_upper_threshold = 0.95;

      CuMatrix<BaseFloat> relu_storage(2, dim_input + 2, kUndefined);
      CuSubVector<BaseFloat> relu_thresholds_vec(relu_storage.RowData(0) + dim_input, 2);
      CuSubMatrix<BaseFloat> relu_stats_mat(relu_storage, 0, 2, 0, dim_input);
      relu_thresholds_vec(0) = -relu_lower_threshold;
      relu_thresholds_vec(1) = -relu_upper_threshold;
      CuSubVector<BaseFloat> relu_row0(relu_stats_mat, 0);
      CuSubVector<BaseFloat> relu_row1(relu_stats_mat, 1);

      CuVector<BaseFloat> relu_deriv_sum_(dim_input);
      relu_deriv_sum_.AddRowSumMat(1.0, relu_in_deriv, 1.0);

      relu_row0.CopyFromVec(relu_deriv_sum_);
      relu_row1.CopyFromVec(relu_row0);
      relu_stats_mat.AddVecToCols(1.0, relu_thresholds_vec, 1.0);
      relu_stats_mat.ApplyHeaviside();
      relu_row0.AddVec(1.0, relu_row1, 1.0);
      relu_row0.Add(-1.0);

      relu_row0.Scale(-relu_self_repair_scale_ / relu_repair_probability);
      relu_in_deriv.AddVecToRows(1.0, relu_row0, 1.0);
    }

    // //Tanh
    CuMatrix<BaseFloat> tanh_out_value(in_value.NumRows(), in_value.NumCols());
    CuMatrix<BaseFloat> tanh_in_deriv(in_value.NumRows(), in_value.NumCols());
    CuMatrix<BaseFloat> tanh_out_deriv(out_deriv);
    CuSubVector<BaseFloat> tanh_scale_(activation_params, 2);

    tanh_out_value.Tanh(in_value);
    tanh_out_deriv.MulColsVec(tanh_scale_);

    tanh_in_deriv.DiffTanh(tanh_out_value, tanh_out_deriv);

    CuVector<BaseFloat> tanh_deriv_sum_(dim_input);
    tanh_deriv_sum_.AddRowSumMat(1.0, tanh_in_deriv, 1.0);

    BaseFloat tanh_repair_scale_ = 0.00001;
    BaseFloat tanh_repair_probability = 0.5;
    BaseFloat tanh_lower_threshold = 0.2;

    if (RandUniform() <= tanh_repair_probability) {
      CuMatrix<BaseFloat> tanh_thresholds(1, dim_input);
      CuSubVector<BaseFloat> tanh_thresholds_vec(tanh_thresholds, 0);
      tanh_thresholds_vec.AddVec(-1.0, tanh_deriv_sum_);
      tanh_thresholds_vec.Add(tanh_lower_threshold);
      tanh_thresholds.ApplyHeaviside();

      tanh_in_deriv.AddMatDiagVec(-tanh_repair_scale_ / tanh_repair_probability,
                          tanh_out_value, kNoTrans, tanh_thresholds_vec);
    }


    in_deriv->AddMat(1.0, sigmoid_in_deriv);
    in_deriv->AddMat(1.0, relu_in_deriv);
    in_deriv->AddMat(1.0, tanh_in_deriv);
  }
  if (to_update != NULL) {
    // Next update the model (must do this 2nd so the derivatives we propagate
    // are accurate, in case this == to_update_in.)
    if (to_update->is_gradient_)
      ...
      to_update->Update(debug_info, in_value, out_deriv, rand_mat_, activation_params, activation_params_mean_, activation_params_std_, activation_prior_mean_, activation_prior_std_);  // by child classes.
  }
}

void GPActivationComponent::Read(std::istream &is, bool binary) {
  CuMatrix<BaseFloat> activation_params_mean_;
  CuMatrix<BaseFloat> activation_params_std_;
  ReadUpdatableCommon(is, binary);  // read opening tag and learning rate.
  ExpectToken(is, binary, "<activationParamsMean>");
  activation_params_mean_.Read(is, binary);
  ExpectToken(is, binary, "<activationParamsStd>");
  activation_params_std_.Read(is, binary);
  ExpectToken(is, binary, "<activationPriorMean>");
  activation_params_.Range(0, activation_params_.NumRows()/2, 0, activation_params_.NumCols()).CopyFromMat(activation_params_mean_);
  activation_params_.Range(activation_params_.NumRows()/2, activation_params_.NumRows()/2, 0, activation_params_.NumCols()).CopyFromMat(activation_params_std_);
  activation_prior_mean_.Read(is, binary);
  ExpectToken(is, binary, "<activationPriorStd>");
  activation_prior_std_.Read(is, binary);
  ExpectToken(is, binary, "<BiasParams>");
  bias_params_.Read(is, binary);
  if (PeekToken(is, binary) == 'I') {
    // for back compatibility; we don't write this here any
  ...
  ExpectToken(is, binary, "<KLScale>");
  ReadBasicType(is, binary, &KL_scale_);
  ExpectToken(is, binary, "</GPActivationComponent>");
}

void GPActivationComponent::Write(std::ostream &os, bool binary) const {
  WriteUpdatableCommon(os, binary);  // Write opening tag and learning rate
  CuSubMatrix<BaseFloat> activation_params_mean_(activation_params_,
                                          0, activation_params_.NumRows()/2,
                                          0, activation_params_.NumCols());
  CuSubMatrix<BaseFloat> activation_params_std_(activation_params_, 
                                        activation_params_.NumRows()/2, activation_params_.NumRows()/2,
                                        0, activation_params_.NumCols());
  WriteToken(os, binary, "<activationParamsMean>");
  activation_params_mean_.Write(os, binary);
  WriteToken(os, binary, "<activationParamsStd>");
  activation_params_std_.Write(os, binary);
  WriteToken(os, binary, "<activationPriorMean>");
  activation_prior_mean_.Write(os, binary);
  WriteToken(os, binary, "<activationPriorStd>");
  activation_prior_std_.Write(os, binary);
  WriteToken(os, binary, "<BiasParams>");
  bias_params_.Write(os, binary);
  if (orthonormal_constraint_ != 0.0) {
    WriteToken(os, binary, "<OrthonormalConstraint>");
  ...
  WriteToken(os, binary, "<KLScale>");
  WriteBasicType(os, binary, KL_scale_);
  WriteToken(os, binary, "</GPActivationComponent>");
}

int32 GPActivationComponent::NumParameters() const {
  int32 in_dim_std = activation_params_.NumCols(), out_dim_std = activation_params_.NumRows()/2;
  return (InputDim()*2 + 1) * OutputDim() + in_dim_std*out_dim_std*2;
}

void GPActivationComponent::Vectorize(VectorBase<BaseFloat> *params) const {
  KALDI_ASSERT(params->Dim() == this->NumParameters());
  ...
  int32 in_dim_std = activation_params_.NumCols(), 
  out_dim_std = activation_params_.NumRows()/2, 
  pose_current;
  
  params->Range(0, InputDim() * OutputDim()).CopyRowsFromMat(activation_params_mean_);
  pose_current = InputDim() * OutputDim();
  params->Range(pose_current, in_dim_std*out_dim_std).CopyRowsFromMat(activation_params_std_);
  pose_current += in_dim_std*out_dim_std;
  // params->Range(0, InputDim() * OutputDim()).CopyRowsFromMat(activation_params_mean_);
  // pose_current = InputDim() * OutputDim();
  // params->Range(pose_current, in_dim_std*out_dim_std).CopyRowsFromMat(activation_params_std_);
  // pose_current += in_dim_std*out_dim_std;
  params->Range(pose_current, InputDim() * OutputDim()).CopyRowsFromMat(activation_prior_mean_);
  pose_current += InputDim() * OutputDim();
  params->Range(pose_current, in_dim_std*out_dim_std).CopyRowsFromMat(activation_prior_std_);
  pose_current += in_dim_std*out_dim_std;
  params->Range(pose_current, OutputDim()).CopyFromVec(bias_params_);
}

void GPActivationComponent::UnVectorize(const VectorBase<BaseFloat> &params) {
  KALDI_ASSERT(params.Dim() == this->NumParameters());
  int32 in_dim_std = activation_params_.NumCols(), 
  out_dim_std = activation_params_.NumRows()/2, 
  pose_current;
  CuMatrix<BaseFloat> activation_params_mean_(out_dim_std, in_dim_std);
  CuMatrix<BaseFloat> activation_params_std_(out_dim_std, in_dim_std);

  activation_params_mean_.CopyRowsFromVec(params.Range(0, InputDim() * OutputDim()));
  pose_current = InputDim() * OutputDim();
  activation_params_std_.CopyRowsFromVec(params.Range(pose_current, in_dim_std*out_dim_std));
  pose_current += in_dim_std*out_dim_std;
  // activation_params_mean_.CopyRowsFromVec(params.Range(0, InputDim() * OutputDim()));
  // pose_current = InputDim() * OutputDim();
  // activation_params_std_.CopyRowsFromVec(params.Range(pose_current, in_dim_std*out_dim_std));
  // pose_current += in_dim_std*out_dim_std;
  activation_prior_mean_.CopyRowsFromVec(params.Range(pose_current, InputDim() * OutputDim()));
  pose_current += InputDim() * OutputDim();
  activation_prior_std_.CopyRowsFromVec(params.Range(pose_current, in_dim_std*out_dim_std));
  pose_current += in_dim_std*out_dim_std;
  bias_params_.CopyFromVec(params.Range(pose_current, OutputDim()));
  activation_params_.Range(0, activation_params_.NumRows()/2, 0, activation_params_.NumCols()).CopyFromMat(activation_params_mean_);
  activation_params_.Range(activation_params_.NumRows()/2, activation_params_.NumRows()/2, 0, activation_params_.NumCols()).CopyFromMat(activation_params_std_);
...
void NaturalGradientGPActivationComponent::Read(std::istream &is, bool binary) {
  ReadUpdatableCommon(is, binary);  // read opening tag and learning rate.
  CuMatrix<BaseFloat> activation_params_mean_;
  CuMatrix<BaseFloat> activation_params_std_;
  ...
  activation_params_.Resize(activation_params_mean_.NumRows()*2, activation_params_mean_.NumCols());
  activation_params_.Range(0, activation_params_.NumRows()/2, 0, activation_params_.NumCols()).CopyFromMat(activation_params_mean_);
  activation_params_.Range(activation_params_.NumRows()/2, activation_params_.NumRows()/2, 0, activation_params_.NumCols()).CopyFromMat(activation_params_std_);
  ...
  ReadToken(is, binary, &token);
  // the following has to handle a couple variants of
  if (token.find("NaturalGradientGPActivationComponent>") == std::string::npos)
    KALDI_ERR << "Expected <NaturalGradientGPActivationComponent> or "
              << "</NaturalGradientGPActivationComponent>, got " << token;
}


NaturalGradientGPActivationComponent::NaturalGradientGPActivationComponent(
    const CuMatrixBase<BaseFloat> &activation_params_,
    const CuMatrixBase<BaseFloat> &activation_prior_mean,
    const CuMatrixBase<BaseFloat> &activation_prior_std,
        const CuVectorBase<BaseFloat> &bias_params):
    GPActivationComponent(activation_params_, activation_prior_mean, activation_prior_std, bias_params, 0.001) {
  KALDI_ASSERT(bias_params.Dim() == activation_params_.NumRows()/2 &&
               bias_params.Dim() != 0);

  // set some default natural gradient configs.
  preconditioner_.SetRank(80);
  preconditioner_.SetUpdatePeriod(4);
}

void NaturalGradientGPActivationComponent::InitFromConfig(ConfigLine *cfl) {
  bool ok = true;
  std::string matrix_filename;
  ...
  // }
    ...
    activation_params_.Range(0, activation_params_.NumRows()/2, 0, activation_params_.NumCols()).CopyFromMat(activation_params_mean_);
    activation_params_.Range(activation_params_.NumRows()/2, activation_params_.NumRows()/2, 0, activation_params_.NumCols()).CopyFromMat(activation_params_std_);    
    activation_prior_mean_.CopyFromMat(mat.Range(0, output_dim, 2*input_dim, input_dim));
    activation_prior_std_.CopyFromMat(mat.Range(0, output_dim_re, 3*input_dim, input_dim_re));
    bias_params_.CopyColFromMat(mat, 4*input_dim);
    ...
    if (!ok)
      KALDI_ERR << "Bad initializer " << cfl->WholeLine();
    // BaseFloat param_mean_stddev = 1.0 / std::sqrt(input_dim),
    // BaseFloat param_mean_stddev = 0.0;
    BaseFloat param_mean = 0.33;
    BaseFloat param_mean_stddev = 1.0 / std::sqrt(input_dim),
    param_std_stddev = 1.0 / std::sqrt(input_dim),
    prior_mean = 0.5, prior_std,
        bias_stddev = 1.0, bias_mean = 0.0;
    if (!use_exp_std_) {
      prior_std = 0.1;
    } else {
    ...
    //   output_dim_re = 1;
    // }
    activation_params_.Resize(output_dim_re*2, input_dim);
    CuMatrix<BaseFloat> activation_params_mean_(output_dim_re, input_dim);
    CuMatrix<BaseFloat> activation_params_std_(output_dim_re, input_dim);
    activation_prior_mean_.Resize(output_dim_re, input_dim);
    activation_prior_std_.Resize(output_dim_re, input_dim_re);
    ...
    activation_params_mean_.SetRandn(); // sets to random normally distributed noise.
    activation_params_mean_.Scale(param_mean_stddev);
    activation_params_mean_.Add(param_mean);
  
    activation_params_std_.SetRandn(); // sets to random normally distributed noise.
    activation_params_std_.Scale(param_std_stddev);
    if (!use_exp_std_) {
      activation_params_std_.Add(-2.0); // init around -1
    }
  ...
  preconditioner_.SetNumSamplesHistory(num_samples_history);
  preconditioner_.SetAlpha(alpha);
  preconditioner_.SetRank(rank_in);
  preconditioner_.SetUpdatePeriod(update_period);
...
void NaturalGradientGPActivationComponent::Write(std::ostream &os,
                                           bool binary) const {
  WriteUpdatableCommon(os, binary);  // Write opening tag and learning rate
  CuSubMatrix<BaseFloat> activation_params_mean_(activation_params_,
                                          0, activation_params_.NumRows()/2,
  ...
  WriteToken(os, binary, "<Alpha>");
  WriteBasicType(os, binary, preconditioner_.GetAlpha());
  WriteToken(os, binary, "</NaturalGradientGPActivationComponent>");
}

std::string NaturalGradientGPActivationComponent::Info() const {
  std::ostringstream stream;
  stream << GPActivationComponent::Info();
  stream << ", rank" << preconditioner_.GetRank()
         << ", num-samples-history=" << preconditioner_.GetNumSamplesHistory()
         << ", update-period=" << preconditioner_.GetUpdatePeriod()
         << ", alpha=" << preconditioner_.GetAlpha();
  return stream.str();
...
Component* NaturalGradientGPActivationComponent::Copy() const {
  return new NaturalGradientGPActivationComponent(*this);
}

NaturalGradientGPActivationComponent::NaturalGradientGPActivationComponent(
    const NaturalGradientGPActivationComponent &other):
    GPActivationComponent(other),
    preconditioner_(other.preconditioner_) { }

void NaturalGradientGPActivationComponent::Update(
    const std::string &debug_info,
    const CuMatrixBase<BaseFloat> &in_value,
    const CuMatrixBase<BaseFloat> &out_deriv,
    const CuMatrixBase<BaseFloat> &rand_mat,
    const CuMatrixBase<BaseFloat> &activation_params_norm_tmp_,
    const CuMatrixBase<BaseFloat> &activation_params_mean_tmp_,
    const CuMatrixBase<BaseFloat> &activation_params_std_tmp_,
    const CuMatrixBase<BaseFloat> &activation_prior_mean_tmp_,
    const CuMatrixBase<BaseFloat> &activation_prior_std_tmp_) {

  // alpha_1=e^coef_1 / sum_k(coef_k), alpha_2=e^coef_2 / sum_k(coef_k), alpha_3=e^coef_3 / sum_k(coef_k)
  // alpha_4=e^coef_1 * e^coef_1, alpha_5=e^coef_1 * e^coef_2, alpha_6=e^coef_1 * e^coef_3
  // alpha_7=e^coef_2 * e^coef_2, alpha_8=e^coef_2 * e^coef_3, alpha_9=e^coef_3 * e^coef_3
  CuSubMatrix<BaseFloat> alpha_1(activation_params_norm_tmp_, 
                                        0, 1,
                                        0, activation_params_norm_tmp_.NumCols());

  CuSubMatrix<BaseFloat> alpha_2(activation_params_norm_tmp_, 
                                        0, 1,
                                        0, activation_params_norm_tmp_.NumCols());

  CuSubMatrix<BaseFloat> alpha_3(activation_params_norm_tmp_, 
                                        0, 1,
                                        0, activation_params_norm_tmp_.NumCols());
 
  CuMatrix<BaseFloat> alpha_4(1, in_value.NumCols());
  CuMatrix<BaseFloat> alpha_5(1, in_value.NumCols());
  CuMatrix<BaseFloat> alpha_6(1, in_value.NumCols());
  CuMatrix<BaseFloat> alpha_7(1, in_value.NumCols());
  CuMatrix<BaseFloat> alpha_8(1, in_value.NumCols());
  CuMatrix<BaseFloat> alpha_9(1, in_value.NumCols());

  alpha_4.CopyFromMat(alpha_1);
  alpha_4.MulElements(alpha_1);

  alpha_5.CopyFromMat(alpha_1);
  alpha_5.MulElements(alpha_2);

  alpha_6.CopyFromMat(alpha_1);
  alpha_6.MulElements(alpha_3);

  alpha_7.CopyFromMat(alpha_2);
  alpha_7.MulElements(alpha_2);

  alpha_8.CopyFromMat(alpha_2);
  alpha_8.MulElements(alpha_3);

  alpha_9.CopyFromMat(alpha_3);
  alpha_9.MulElements(alpha_3); 

  // in_value after activation
  CuMatrix<BaseFloat> sigmoid_out(in_value.NumRows(), in_value.NumCols());
  CuMatrix<BaseFloat> relu_out(in_value.NumRows(), in_value.NumCols());
  CuMatrix<BaseFloat> tanh_out(in_value.NumRows(), in_value.NumCols());  

  sigmoid_out.Sigmoid(in_value);
  relu_out.CopyFromMat(in_value);
  relu_out.ApplyFloor(0.0);
  tanh_out.Tanh(in_value);

  // // update sigmoid coefficients
  CuMatrix<BaseFloat> sigmoid_derivs_per_frame_temp(in_value.NumRows(), in_value.NumCols());

  CuMatrix<BaseFloat> sig_sigmoid_alpha_1(in_value.NumRows(), in_value.NumCols());
  CuMatrix<BaseFloat> sig_relu_alpha_1(in_value.NumRows(), in_value.NumCols());
  CuMatrix<BaseFloat> sig_tanh_alpha_1(in_value.NumRows(), in_value.NumCols());

  sig_sigmoid_alpha_1.AddMatBlocks(1.0, alpha_1, kNoTrans);
  sig_sigmoid_alpha_1.AddMatBlocks(-1.0, alpha_4, kNoTrans);
  sig_sigmoid_alpha_1.MulElements(sigmoid_out);

  sig_relu_alpha_1.AddMatBlocks(1.0, alpha_5, kNoTrans);  
  sig_relu_alpha_1.MulElements(relu_out);

  sig_tanh_alpha_1.AddMatBlocks(1.0, alpha_6, kNoTrans);  
  sig_tanh_alpha_1.MulElements(tanh_out);  

  sigmoid_derivs_per_frame_temp.CopyFromMat(sig_sigmoid_alpha_1);
  sigmoid_derivs_per_frame_temp.AddMat(-1.0, sig_relu_alpha_1);
  sigmoid_derivs_per_frame_temp.AddMat(-1.0, sig_tanh_alpha_1);

  sigmoid_derivs_per_frame_temp.MulElements(out_deriv);

  BaseFloat scale;
  preconditioner_.PreconditionDirections(&sigmoid_derivs_per_frame_temp, &scale); 

  CuVector<BaseFloat> sigmoid_scales_mean(activation_params_mean_tmp_.NumCols());
  CuVector<BaseFloat> sigmoid_scales_std(activation_params_mean_tmp_.NumCols());
  CuVector<BaseFloat> sigmoid_delta_scales_mean(activation_params_mean_tmp_.NumCols());
  CuVector<BaseFloat> sigmoid_delta_scales_std(activation_params_mean_tmp_.NumCols());
  sigmoid_delta_scales_mean.AddRowSumMat(scale * learning_rate_, sigmoid_derivs_per_frame_temp);
  // update sigmoid scale mean
  sigmoid_scales_mean.AddVec(1.0, sigmoid_delta_scales_mean);

  // update sigmoid scale std
  CuSubMatrix<BaseFloat> sigmoid_rand_mat(rand_mat, 
                                        0, 1,
                                        0, rand_mat.NumCols());
  CuMatrix<BaseFloat> sigmoid_rand_mat_expand(in_value.NumRows(), in_value.NumCols());
  sigmoid_rand_mat_expand.AddMatBlocks(1.0, sigmoid_rand_mat, kNoTrans);

  sigmoid_derivs_per_frame_temp.MulElements(sigmoid_rand_mat_expand);

  CuSubMatrix<BaseFloat> sigmoid_params_std_(activation_params_std_tmp_, 
                                        0, 1,
                                        0, activation_params_std_tmp_.NumCols());

  CuMatrix<BaseFloat> sigmoid_params_std_expand_(in_value.NumRows(), in_value.NumCols());
  sigmoid_params_std_expand_.AddMatBlocks(1.0, sigmoid_params_std_, kNoTrans);

  if (use_exp_std_) {
    CuMatrix<BaseFloat> sigmoid_params_std_exp(sigmoid_params_std_expand_);
    sigmoid_params_std_exp.CopyFromMat(sigmoid_params_std_expand_);
    sigmoid_params_std_exp.ApplyCeiling(46.05);
    sigmoid_params_std_exp.ApplyExp();
    sigmoid_derivs_per_frame_temp.MulElements(sigmoid_params_std_exp);
  } 

  sigmoid_delta_scales_std.AddRowSumMat(scale * learning_rate_, sigmoid_derivs_per_frame_temp);
  sigmoid_scales_std.AddVec(1.0, sigmoid_delta_scales_std);


  // update relu parameters
  CuMatrix<BaseFloat> relu_derivs_per_frame_temp(in_value.NumRows(), in_value.NumCols());

  CuMatrix<BaseFloat> rel_sigmoid_alpha_2(in_value.NumRows(), in_value.NumCols());
  CuMatrix<BaseFloat> rel_relu_alpha_2(in_value.NumRows(), in_value.NumCols());
  CuMatrix<BaseFloat> rel_tanh_alpha_2(in_value.NumRows(), in_value.NumCols());

  rel_sigmoid_alpha_2.AddMatBlocks(1.0, alpha_5, kNoTrans);
  rel_sigmoid_alpha_2.MulElements(sigmoid_out);

  rel_relu_alpha_2.AddMatBlocks(1.0, alpha_2, kNoTrans);
  rel_relu_alpha_2.AddMatBlocks(-1.0, alpha_7, kNoTrans);  
  rel_relu_alpha_2.MulElements(relu_out);

  rel_tanh_alpha_2.AddMatBlocks(1.0, alpha_8, kNoTrans);  
  rel_tanh_alpha_2.MulElements(tanh_out);  

  relu_derivs_per_frame_temp.CopyFromMat(rel_relu_alpha_2);
  relu_derivs_per_frame_temp.AddMat(-1.0, rel_sigmoid_alpha_2);
  relu_derivs_per_frame_temp.AddMat(-1.0, rel_tanh_alpha_2);

  relu_derivs_per_frame_temp.MulElements(out_deriv);

  preconditioner_.PreconditionDirections(&relu_derivs_per_frame_temp, &scale); 

  CuVector<BaseFloat> relu_scales_mean(activation_params_mean_tmp_.NumCols());
  CuVector<BaseFloat> relu_scales_std(activation_params_mean_tmp_.NumCols());
  CuVector<BaseFloat> relu_delta_scales_mean(activation_params_mean_tmp_.NumCols());
  CuVector<BaseFloat> relu_delta_scales_std(activation_params_mean_tmp_.NumCols());
  relu_delta_scales_mean.AddRowSumMat(scale * learning_rate_, relu_derivs_per_frame_temp);
  // update relu scale mean
  relu_scales_mean.AddVec(1.0, relu_delta_scales_mean);

  // update relu scale std
  CuSubMatrix<BaseFloat> relu_rand_mat(rand_mat, 
                                        1, 1,
                                        0, rand_mat.NumCols());
  CuMatrix<BaseFloat> relu_rand_mat_expand(in_value.NumRows(), in_value.NumCols());
  relu_rand_mat_expand.AddMatBlocks(1.0, relu_rand_mat, kNoTrans);

  relu_derivs_per_frame_temp.MulElements(relu_rand_mat_expand);

  CuSubMatrix<BaseFloat> relu_params_std_(activation_params_std_tmp_, 
                                        1, 1,
                                        0, activation_params_std_tmp_.NumCols());

  CuMatrix<BaseFloat> relu_params_std_expand_(in_value.NumRows(), in_value.NumCols());
  relu_params_std_expand_.AddMatBlocks(1.0, relu_params_std_, kNoTrans);

  if (use_exp_std_) {
    CuMatrix<BaseFloat> relu_params_std_exp(relu_params_std_expand_);
    relu_params_std_exp.CopyFromMat(relu_params_std_expand_);
    relu_params_std_exp.ApplyCeiling(46.05);
    relu_params_std_exp.ApplyExp();
    relu_derivs_per_frame_temp.MulElements(relu_params_std_exp);
  } 

  relu_delta_scales_std.AddRowSumMat(scale * learning_rate_, relu_derivs_per_frame_temp);
  relu_scales_std.AddVec(1.0, relu_delta_scales_std);


  // // update tanh parameters
  CuMatrix<BaseFloat> tanh_derivs_per_frame_temp(in_value.NumRows(), in_value.NumCols());

  CuMatrix<BaseFloat> tan_sigmoid_alpha_3(in_value.NumRows(), in_value.NumCols());
  CuMatrix<BaseFloat> tan_relu_alpha_3(in_value.NumRows(), in_value.NumCols());
  CuMatrix<BaseFloat> tan_tanh_alpha_3(in_value.NumRows(), in_value.NumCols());

  tan_sigmoid_alpha_3.AddMatBlocks(1.0, alpha_6, kNoTrans);
  tan_sigmoid_alpha_3.MulElements(sigmoid_out);

  tan_relu_alpha_3.AddMatBlocks(1.0, alpha_8, kNoTrans); 
  tan_relu_alpha_3.MulElements(relu_out);

  tan_tanh_alpha_3.AddMatBlocks(1.0, alpha_3, kNoTrans); 
  tan_tanh_alpha_3.AddMatBlocks(-1.0, alpha_9, kNoTrans);  
  tan_tanh_alpha_3.MulElements(tanh_out);  

  tanh_derivs_per_frame_temp.CopyFromMat(tan_tanh_alpha_3);
  tanh_derivs_per_frame_temp.AddMat(-1.0, tan_relu_alpha_3);
  tanh_derivs_per_frame_temp.AddMat(-1.0, tan_sigmoid_alpha_3);

  tanh_derivs_per_frame_temp.MulElements(out_deriv);

  preconditioner_.PreconditionDirections(&tanh_derivs_per_frame_temp, &scale); 

  CuVector<BaseFloat> tanh_scales_mean(activation_params_mean_tmp_.NumCols());
  CuVector<BaseFloat> tanh_scales_std(activation_params_mean_tmp_.NumCols());
  CuVector<BaseFloat> tanh_delta_scales_mean(activation_params_mean_tmp_.NumCols());
  CuVector<BaseFloat> tanh_delta_scales_std(activation_params_mean_tmp_.NumCols());
  tanh_delta_scales_mean.AddRowSumMat(scale * learning_rate_, tanh_derivs_per_frame_temp);
  // update tanh scale mean
  tanh_scales_mean.AddVec(1.0, tanh_delta_scales_mean);

  // update tanh scale std
  CuSubMatrix<BaseFloat> tanh_rand_mat(rand_mat, 
                                        2, 1,
                                        0, rand_mat.NumCols());
  CuMatrix<BaseFloat> tanh_rand_mat_expand(in_value.NumRows(), in_value.NumCols());
  tanh_rand_mat_expand.AddMatBlocks(1.0, tanh_rand_mat, kNoTrans);

  tanh_derivs_per_frame_temp.MulElements(tanh_rand_mat_expand);

  CuSubMatrix<BaseFloat> tanh_params_std_(activation_params_std_tmp_, 
                                        2, 1,
                                        0, activation_params_std_tmp_.NumCols());

  CuMatrix<BaseFloat> tanh_params_std_expand_(in_value.NumRows(), in_value.NumCols());

  tanh_params_std_expand_.AddMatBlocks(1.0, tanh_params_std_, kNoTrans);

  if (use_exp_std_) {
    CuMatrix<BaseFloat> tanh_params_std_exp(tanh_params_std_expand_);
    tanh_params_std_exp.CopyFromMat(tanh_params_std_expand_);
    tanh_params_std_exp.ApplyCeiling(46.05);
    tanh_params_std_exp.ApplyExp();
    tanh_derivs_per_frame_temp.MulElements(tanh_params_std_exp);
  } 

  tanh_delta_scales_std.AddRowSumMat(scale * learning_rate_, tanh_derivs_per_frame_temp);
  tanh_scales_std.AddVec(1.0, tanh_delta_scales_std);

  CuMatrix<BaseFloat> activation_params_mean_(activation_params_mean_tmp_.NumRows(), activation_params_mean_tmp_.NumCols());
  CuMatrix<BaseFloat> activation_params_std_(activation_params_std_tmp_.NumRows(), activation_params_std_tmp_.NumCols());

  activation_params_mean_.Range(0, 1, 0, activation_params_mean_.NumCols()).CopyRowsFromVec(sigmoid_scales_mean);
  activation_params_std_.Range(0, 1, 0, activation_params_mean_.NumCols()).CopyRowsFromVec(sigmoid_scales_std);
  activation_params_mean_.Range(1, 1, 0, activation_params_mean_.NumCols()).CopyRowsFromVec(relu_scales_mean);
  activation_params_std_.Range(1, 1, 0, activation_params_mean_.NumCols()).CopyRowsFromVec(relu_scales_std);
  activation_params_mean_.Range(2, 1, 0, activation_params_mean_.NumCols()).CopyRowsFromVec(tanh_scales_mean);
  activation_params_std_.Range(2, 1, 0, activation_params_mean_.NumCols()).CopyRowsFromVec(tanh_scales_std);
  activation_params_mean_.Scale(5.0);

  int32 dim_output;
  int32 dim_input;
  dim_output = activation_params_mean_tmp_.NumRows();
  dim_input = activation_params_mean_tmp_.NumCols();

  CuMatrix<BaseFloat> activation_prior_std_tmp_set_(dim_output, dim_input);
  activation_prior_std_tmp_set_.CopyFromMat(activation_prior_std_tmp_);
  //activation_prior_std_tmp_set_.Set(-3.0);
  ...
  if (KL_scale_ != 0) {
    ...
    CuMatrix<BaseFloat> activation_params_std_resize(dim_output, dim_input);
    CuMatrix<BaseFloat> activation_prior_std_resize(dim_output, dim_input);
    activation_params_std_resize.AddMatBlocks(1.0, activation_params_std_tmp_, kNoTrans);
    activation_prior_std_resize.AddMatBlocks(1.0, activation_prior_std_tmp_set_, kNoTrans);
    ...
    diff.CopyFromMat(activation_params_mean_tmp_);
    diff.AddMat(-1.0, activation_prior_mean_tmp_); // post_mean - prior_mean
    diff.MulElements(inv_prior_std); // (post_mean - prior_mean)/prior_std
...
void NaturalGradientGPActivationComponent::Scale(BaseFloat scale) {
  if (scale == 0.0) {
    // If scale == 0.0 we call SetZero() which will get rid of NaN's and inf's.
    activation_params_.SetZero();
    activation_prior_mean_.SetZero();
...
void NaturalGradientGPActivationComponent::Add(BaseFloat alpha, const Component &other_in) {
  const NaturalGradientGPActivationComponent *other =
      dynamic_cast<const NaturalGradientGPActivationComponent*>(&other_in);
  KALDI_ASSERT(other != NULL);
  activation_params_.AddMat(alpha, other->activation_params_);
  activation_prior_mean_.AddMat(alpha, other->activation_prior_mean_);
  activation_prior_std_.AddMat(alpha, other->activation_prior_mean_);
  bias_params_.AddVec(alpha, other->bias_params_);
}

void NaturalGradientGPActivationComponent::FreezeNaturalGradient(bool freeze) {
  preconditioner_.Freeze(freeze);
}

void NaturalGradientGPActivationComponent::ConsolidateMemory() {
  OnlineNaturalGradient temp_(preconditioner_);
  preconditioner_.Swap(&temp_);
}
...
//   // CuVector<BaseFloat> vec_mean1(linear_params_mean_.NumCols());
//   // vec_mean1.AddRowSumMat(1.0, linear_params_mean_, 1.0);
//   // KALDI_LOG << "linear_params_mean_: " << vec_mean1.Sum();

//   // CuVector<BaseFloat> vec_sum1(linear_params_std_.NumCols());
//   // vec_sum1.AddRowSumMat(1.0, linear_params_std_, 1.0);
//   // KALDI_LOG << "Hello: " << vec_sum1.Sum();
...
//     // CuMatrix<BaseFloat> linear_params_std_exp(linear_params_std_);
//     CuMatrix<BaseFloat> linear_params_std_exp(dim_output, dim_input);
//     linear_params_std_exp.CopyFromMat(linear_params_std_);
//     linear_params_std_exp.ApplyCeiling(46.05);
//     linear_params_std_exp.ApplyExp();
//     linear_deriv.MulElements(linear_params_std_exp);
//     // KALDI_LOG << "linear_deriv " << linear_deriv.NumRows() << " " << linear_deriv.NumCols();
//   }
//   // CuVector<BaseFloat> vec_std(linear_deriv.NumCols());
//   // vec_std.AddRowSumMat(1.0, linear_deriv, 1.0);

//   // CuMatrix<BaseFloat> linear_params_std_tmp(1, dim_input);
//   // linear_params_std_.AddVecToRows(1.0, vec_std, 1.0);
...
//   // float value1=linear_params_std_(0,0);
//   // KALDI_LOG << "value1: " << value1;
//   // float value2=linear_params_std_(0,1);
//   // KALDI_LOG << "value2: " << value2;

//   // KALDI_LOG << "linear_params_std_ " << linear_params_std_.NumRows() << " " << linear_params_std_.NumCols();
//   // CuVector<BaseFloat> vec_mean(linear_params_mean_.NumCols());
//   // vec_mean.AddRowSumMat(1.0, linear_params_mean_, 1.0);
//   // KALDI_LOG << "linear_params_mean_: " << vec_mean.Sum();
//   // float value=linear_params_std_(0,0);
//   // KALDI_LOG << "value: " << value;

//   // CuVector<BaseFloat> vec_sum(linear_params_std_.NumCols());
//   // // KALDI_LOG << vec_sum.Dim();
//   // vec_sum.AddRowSumMat(1.0, linear_params_std_, 1.0);
//   // KALDI_LOG << "linear_params_std_: " << vec_sum.Sum();
//   // KALDI_LOG << "linear_params_std_ " << linear_params_std_.NumRows() << " " << linear_params_std_.NumCols();
...
/////////////////////////////////////////////////////////////////////////////////////////////
FlopsConstraintComponent::FlopsConstraintComponent(const FlopsConstraintComponent &other):
    // input_dim_(other.input_dim_), output_dim_(other.output_dim_),
    // scale_(other.scale_) { }
    input_dim_(other.input_dim_),  output_dim_(other.output_dim_), scale_(other.scale_), 
    flops_(other.flops_), cuflops_(other.cuflops_) { }

void FlopsConstraintComponent::InitFromConfig(ConfigLine *cfl) {
  scale_ = 1.0;
  std::string flops;
  bool ok = cfl->GetValue("input-dim", &input_dim_) && 
    cfl->GetValue("output-dim", &output_dim_) &&
    cfl->GetValue("flops", &flops);

  if (!ok || input_dim_ <= 0 || output_dim_ <= 0 ||
      !SplitStringToIntegers(flops, ",", false, &flops_)) {
    KALDI_ERR << "Bad initializer: there is a problem with "
        "flops, input-dim and output-dim (not defined?): "
        << cfl->WholeLine();
  }
  Vector<BaseFloat> flops_mid(flops_.size());
  for (int32 i = 0; i < flops_.size(); i++) {
      flops_mid.Range(i, 1).Set(flops_[i]);
  }
  cuflops_ = flops_mid;
  // bool ok = cfl->GetValue("input-dim", &input_dim_) &&
  //     cfl->GetValue("output-dim", &output_dim_);
...
void FlopsConstraintComponent::Read(std::istream &is, bool binary) {
  ExpectOneOrTwoTokens(is, binary, "<FlopsConstraintComponent>", "<InputDim>");
  ReadBasicType(is, binary, &input_dim_);
  ExpectToken(is, binary, "<OutputDim>");
  ReadBasicType(is, binary, &output_dim_);
  ExpectToken(is, binary, "<Flops>");
  ReadIntegerVector(is, binary, &flops_);
  ExpectToken(is, binary, "<Scale>");
  ReadBasicType(is, binary, &scale_);
  ExpectToken(is, binary, "</FlopsConstraintComponent>");
}

void FlopsConstraintComponent::Write(std::ostream &os, bool binary) const {
  WriteToken(os, binary, "<FlopsConstraintComponent>");
  WriteToken(os, binary, "<InputDim>");
  WriteBasicType(os, binary, input_dim_);
  WriteToken(os, binary, "<OutputDim>");
  WriteBasicType(os, binary, output_dim_);
  WriteToken(os, binary, "<Flops>");
  WriteIntegerVector(os, binary, flops_);
  WriteToken(os, binary, "<Scale>");
  WriteBasicType(os, binary, scale_);
  WriteToken(os, binary, "</FlopsConstraintComponent>");
}

std::string FlopsConstraintComponent::Info() const {
  std::ostringstream stream;
  stream << Type() << ", input-dim=" << input_dim_
         << ", output-dim=" << output_dim_
         << ", scale=" << scale_;
...
void* FlopsConstraintComponent::Propagate(const ComponentPrecomputedIndexes *indexes,
                                   const CuMatrixBase<BaseFloat> &in,
                                   CuMatrixBase<BaseFloat> *out) const {
  // KALDI_ASSERT(out->NumRows() == in.NumRows() &&
  //              out->NumCols() == output_dim_ &&
...
void FlopsConstraintComponent::Backprop(
    const std::string &debug_info,
    const ComponentPrecomputedIndexes *indexes,
    ...
    CuMatrixBase<BaseFloat> *in_deriv) const {
  if (in_deriv) {
    in_deriv->CopyRowsFromVec(cuflops_);
    in_deriv->Scale(scale_/in_value.NumRows()/in_value.NumCols());
  }
}

//////////////////////////////////////////////////////////////////////////////////
std::string OnehotFunctionComponent::Info() const {
  std::ostringstream stream;
  stream << UpdatableComponent::Info()
         << ", " << Type() << ", input-dim=" << InputDim()
         << ", output-dim=" << OutputDim()
...
OnehotFunctionComponent::OnehotFunctionComponent():
    UpdatableComponent(), input_dim_(-1), is_updatable_(true),
    use_natural_gradient_(true) { }

OnehotFunctionComponent::OnehotFunctionComponent(
    const OnehotFunctionComponent &other):
    UpdatableComponent(other), input_dim_(other.input_dim_),
    output_(other.output_), is_updatable_(other.is_updatable_),
    use_natural_gradient_(other.use_natural_gradient_),
    preconditioner_(other.preconditioner_) { }

void* OnehotFunctionComponent::Propagate(
    const ComponentPrecomputedIndexes *indexes,
    const CuMatrixBase<BaseFloat> &in,
    CuMatrixBase<BaseFloat> *out) const {
  CuVector<BaseFloat> uniform_(1);
  CuVector<BaseFloat> onehot_(output_.Dim());
  uniform_.SetRandUniform();
  onehot_.SetZero();
  for (int32 i = 0; i < output_.Dim(); i++) {
    if (uniform_.Range(0,1).Max() >= (float)(i)/output_.Dim() && uniform_.Range(0,1).Max() < (float)(i+1)/output_.Dim() ){
      onehot_.Range(i,1).Set(1.0);
    }
  }
  out->CopyRowsFromVec(onehot_);
  return NULL;
}

void OnehotFunctionComponent::Backprop(
    const std::string &debug_info,
    const ComponentPrecomputedIndexes *indexes,
    const CuMatrixBase<BaseFloat> &, // in_value
    const CuMatrixBase<BaseFloat> &, // out_value
  ...
  // input, so the input-derivative is zero.
  if (to_update_in) {
    OnehotFunctionComponent *to_update =
      dynamic_cast<OnehotFunctionComponent*>(to_update_in);
    if (to_update->is_updatable_) {
      // only do the update if the is_updatable_ flag is set.
      KALDI_ASSERT(to_update && to_update->is_updatable_);
      if (to_update->use_natural_gradient_ && !to_update->is_gradient_) {
...
void OnehotFunctionComponent::Read(std::istream &is, bool binary) {
  std::string token;
  ReadToken(is, binary, &token);
  if (token == "<OnehotFunctionComponent>") {
    ReadToken(is, binary, &token);
  }
  if (token == "<LearningRateFactor>") {
    ReadBasicType(is, binary, &learning_rate_factor_);
  ...
  ExpectToken(is, binary, "<UseNaturalGradient>");
  ReadBasicType(is, binary, &use_natural_gradient_);
  ExpectToken(is, binary, "</OnehotFunctionComponent>");
}

void OnehotFunctionComponent::Write(std::ostream &os, bool binary) const {
  WriteUpdatableCommon(os, binary);  // Write the opening tag and learning rate
  WriteToken(os, binary, "<InputDim>");
  WriteBasicType(os, binary, input_dim_);
  WriteToken(os, binary, "<Output>");
  ...
  WriteToken(os, binary, "<UseNaturalGradient>");
  WriteBasicType(os, binary, use_natural_gradient_);
  WriteToken(os, binary, "</OnehotFunctionComponent>");
}

Component* OnehotFunctionComponent::Copy() const {
  return new OnehotFunctionComponent(*this);
}

void OnehotFunctionComponent::Scale(BaseFloat scale) {
  if (is_updatable_) {
    if (scale == 0.0) {
      output_.SetZero();
    } else {
...
void OnehotFunctionComponent::Add(BaseFloat alpha, const Component &other_in) {
  if (is_updatable_) {
    const OnehotFunctionComponent *other =
        dynamic_cast<const OnehotFunctionComponent*>(&other_in);
    KALDI_ASSERT(other != NULL);
    output_.AddVec(alpha, other->output_);
  }
}

void OnehotFunctionComponent::PerturbParams(BaseFloat stddev) {
  CuVector<BaseFloat> temp_output(output_.Dim(), kUndefined);
  temp_output.SetRandn();
  output_.AddVec(stddev, temp_output);
}

BaseFloat OnehotFunctionComponent::DotProduct(
    const UpdatableComponent &other_in) const {
  KALDI_ASSERT(is_updatable_);
  const OnehotFunctionComponent *other =
      dynamic_cast<const OnehotFunctionComponent*>(&other_in);
...
void OnehotFunctionComponent::InitFromConfig(ConfigLine *cfl) {
  int32 output_dim = 0;
  InitLearningRatesFromConfig(cfl);
  bool ok = cfl->GetValue("output-dim", &output_dim) &&
      cfl->GetValue("input-dim", &input_dim_);
...
int32 OnehotFunctionComponent::NumParameters() const {
  KALDI_ASSERT(is_updatable_);
  return output_.Dim();
}

void OnehotFunctionComponent::Vectorize(VectorBase<BaseFloat> *params) const {
  params->CopyFromVec(output_);
}

void OnehotFunctionComponent::UnVectorize(const VectorBase<BaseFloat> &params) {
  output_.CopyFromVec(params);
}

void OnehotFunctionComponent::ConsolidateMemory() {
  OnlineNaturalGradient temp(preconditioner_);
  preconditioner_.Swap(&temp);
}

////////////////////////////////////////////////////////////////////////////////////
void* SoftmaxgradnormComponent::Propagate(const ComponentPrecomputedIndexes *indexes,
                                 const CuMatrixBase<BaseFloat> &in,
                                 CuMatrixBase<BaseFloat> *out) const {
  // Apply softmax function to each row of the output...
  // for that row, we do
...
void SoftmaxgradnormComponent::Backprop(const std::string &debug_info,
                                const ComponentPrecomputedIndexes *indexes,
                                const CuMatrixBase<BaseFloat> &, // in_value,
                                const CuMatrixBase<BaseFloat> &out_value,
                                const CuMatrixBase<BaseFloat> &out_deriv,
  ...
  if (to_update_in) {
    SoftmaxgradnormComponent *to_update =
        dynamic_cast<SoftmaxgradnormComponent*>(to_update_in);
    to_update->StoreBackpropStats(out_deriv);
  }
  ...
  */
  in_deriv->DiffSoftmaxPerRow(out_value, out_deriv);
  in_deriv->Scale(100/out_value.NumCols());
}

void SoftmaxgradnormComponent::StoreStats(const CuMatrixBase<BaseFloat> &in_value,
                                  const CuMatrixBase<BaseFloat> &out_value,
                                  void *memo) {
  // We don't store derivative stats for this component type, just activation
  // stats.
...
void GumbelSoftmaxComponent::Init(int32 dim, BaseFloat temp_proportion) {
  temp_proportion_ = temp_proportion;
  dim_ = dim;
}

void GumbelSoftmaxComponent::InitFromConfig(ConfigLine *cfl) {
  int32 dim = 0;
  BaseFloat temp_proportion = 0.0;
  bool ok = cfl->GetValue("dim", &dim) &&
    cfl->GetValue("temp-proportion", &temp_proportion);
    // for this stage, dropout is hard coded in
    // normal mode if not declared in config
  // if (!ok || cfl->HasUnusedValues() || dim <= 0 ||
  //     temp_proportion < 0.0 || temp_proportion > 1.0)
  //      KALDI_ERR << "Invalid initializer for layer of type "
  //                << Type() << ": \"" << cfl->WholeLine() << "\"";
  Init(dim, temp_proportion);
}

std::string GumbelSoftmaxComponent::Info() const {
  std::ostringstream stream;
  stream << Type() << ", dim=" << dim_
         << ", temp-proportion=" << temp_proportion_;
  return stream.str();
}
...
void* GumbelSoftmaxComponent::Propagate(const ComponentPrecomputedIndexes *indexes,
  ...
  // Apply softmax function to each row of the output...
  // for that row, we do
  // gumbel variable G_i=-log(-log(U_i))
  // x_i = exp((x_i+G_i)/temp) / sum_j exp((x_j+G_j)/temp).
  CuVector<BaseFloat> rand_(in.NumCols());
  ...
  CuMatrix<BaseFloat> output(in.NumRows(),in.NumCols());
  output.CopyFromMat(in);
  output.AddVecToRows(1.0, rand_, 1.0);
  output.Scale(1.0 / temp_proportion_);
  out->ApplySoftMaxPerRow(output);
...
void GumbelSoftmaxComponent::Backprop(const std::string &debug_info,
  ...
  // if (to_update_in) {
  //   GumbelSoftmaxComponent *to_update =
  //       dynamic_cast<GumbelSoftmaxComponent*>(to_update_in);
  //   to_update->StoreBackpropStats(out_deriv);
  // }
  ...
  /*
    Note on the derivative of the softmax function: let it be
    p_i = exp((x_i+G_i)/temp) / sum_j exp((x_j+G_j)/temp)
    The [matrix-valued] Jacobian of this function is
    (diag(p) - p p^T)/temp
    Let the derivative vector at the output be e, and at the input be
    d.  We have
    d = (diag(p) e - p (p^T e))/temp.
    d_i = (p_i e_i - p_i (p^T e))/temp.
  */
  in_deriv->DiffSoftmaxPerRow(out_value, out_deriv);
  in_deriv->Scale(1.0 / temp_proportion_);
}
...
void GumbelSoftmaxComponent::Read(std::istream &is, bool binary) {
  ...
  ReadBasicType(is, binary, &dim_);  // read dimension.
  ReadToken(is, binary, &token);
  KALDI_ASSERT(token == "<TempProportion>");
  ReadBasicType(is, binary, &temp_proportion_);  // read dropout rate
  ReadToken(is, binary, &token);
...
void GumbelSoftmaxComponent::Write(std::ostream &os, bool binary) const {
  ...
  WriteToken(os, binary, "<Dim>");
  WriteBasicType(os, binary, dim_);
  WriteToken(os, binary, "<TempProportion>");
...
/////////////////////////////////////////////////////////////////////////////////
void* ArgmaxOnehotFunctionComponent::Propagate(const ComponentPrecomputedIndexes *indexes,
                                 const CuMatrixBase<BaseFloat> &in,
                                 CuMatrixBase<BaseFloat> *out) const {
  // Apply softmax function to each row of the output...
  // for that row, we do
  // x_i = exp(x_i) / sum_j exp(x_j).
  // cout << "alpha " << in.RowRange(0,1) << endl;  
  // out->ApplySoftMaxPerRow(in);
  out->CopyFromMat(in);
  for (int32 i = 0; i < in.NumCols(); i++) {
    if (in.RowRange(0,1).Max() > in.Range(0,1,i,1).Max() ){
      out->ColRange(i,1).Set(0.0);
    } else {
      out->ColRange(i,1).Set(1.0);
    }
  }

  //cout << "rows " << in.NumRows() << "columns " << in.NumCols();
  // cout << "argmax_onehot " << out->RowRange(0,1) << endl;  
  // cout << "argmax_onehot " << out->RowRange(20,1) << endl;  
  // This floor on the output helps us deal with
  // almost-zeros in a way that doesn't lead to overflow.
  // out->ApplyFloor(1.0e-20);
...
void ArgmaxOnehotFunctionComponent::Backprop(const std::string &debug_info,
                                const ComponentPrecomputedIndexes *indexes,
                                const CuMatrixBase<BaseFloat> &, // in_value,
                                const CuMatrixBase<BaseFloat> &out_value,
                                const CuMatrixBase<BaseFloat> &out_deriv,
  ...
  if (to_update_in) {
    ArgmaxOnehotFunctionComponent *to_update =
        dynamic_cast<ArgmaxOnehotFunctionComponent*>(to_update_in);
    to_update->StoreBackpropStats(out_deriv);
  }
...
void ArgmaxOnehotFunctionComponent::StoreStats(const CuMatrixBase<BaseFloat> &in_value,
                                  const CuMatrixBase<BaseFloat> &out_value,
                                  void *memo) {
  // We don't store derivative stats for this component type, just activation
  // stats.
...
/////////////////////////////////////////////////////////////////////////////////
SoftmaxFlopsComponent::SoftmaxFlopsComponent(const SoftmaxFlopsComponent &other):
    RandomComponent(other),
    dim_(other.dim_),
    scale_(other.scale_) { }

Component* SoftmaxFlopsComponent::Copy() const {
  SoftmaxFlopsComponent *ans = new SoftmaxFlopsComponent(*this);
  return ans;
}

void SoftmaxFlopsComponent::Init(int32 dim, BaseFloat scale) {
  scale_ = scale;
  dim_ = dim;
}

void SoftmaxFlopsComponent::InitFromConfig(ConfigLine *cfl) {
  int32 dim = 0;
  BaseFloat scale = 1.0;
  bool ok = cfl->GetValue("dim", &dim) &&
    cfl->GetValue("scale", &scale);
...
std::string SoftmaxFlopsComponent::Info() const {
  std::ostringstream stream;
  stream << Type() << ", dim=" << dim_
         << ", scale=" << scale_;
  return stream.str();
...
void* SoftmaxFlopsComponent::Propagate(const ComponentPrecomputedIndexes *indexes,
                                 const CuMatrixBase<BaseFloat> &in,
                                 CuMatrixBase<BaseFloat> *out) const {
  // Apply softmax function to each row of the output...
  // for that row, we do
...
void SoftmaxFlopsComponent::Backprop(const std::string &debug_info,
                                const ComponentPrecomputedIndexes *indexes,
                                const CuMatrixBase<BaseFloat> &in_value,
                                const CuMatrixBase<BaseFloat> &out_value,
                                const CuMatrixBase<BaseFloat> &out_deriv,
  ...
  */
  ...
  CuVector<BaseFloat> flops_(dim_);
  flops_.Range(0,1).Set(-25);
  flops_.Range(1,1).Set(-50);
  flops_.Range(2,1).Set(-80);
  flops_.Range(3,1).Set(-100);
  flops_.Range(4,1).Set(-120);
  flops_.Range(5,1).Set(-160);
  flops_.Range(6,1).Set(-200);
  flops_.Range(7,1).Set(-240);

  out_deriv.Range(0,out_deriv.NumRows(),0,dim_).AddVecToRows(scale_/out_deriv.NumRows()/out_deriv.NumCols(), flops_, 1.0);
  in_deriv->DiffSoftmaxPerRow(out_value, out_deriv);

  //in_deriv->AddVecToRows(1.0, flops_, scale_/in_value.NumRows()/in_value.NumCols());
}

void SoftmaxFlopsComponent::Read(std::istream &is, bool binary) {
  std::string token;
  ReadToken(is, binary, &token);
  if (token == "<SoftmaxFlopsComponent>") {
    ReadToken(is, binary, &token);
  }
  KALDI_ASSERT(token == "<Dim>");
  ReadBasicType(is, binary, &dim_);  // read dimension.
  ...
  ReadBasicType(is, binary, &scale_);  // read dropout rate
  ReadToken(is, binary, &token);
  KALDI_ASSERT(token == "</SoftmaxFlopsComponent>");
}

void SoftmaxFlopsComponent::Write(std::ostream &os, bool binary) const {
  WriteToken(os, binary, "<SoftmaxFlopsComponent>");
  WriteToken(os, binary, "<Dim>");
  WriteBasicType(os, binary, dim_);
  WriteToken(os, binary, "<Scale>");
  WriteBasicType(os, binary, scale_);
...
GumbelSoftmaxFlopsComponent::GumbelSoftmaxFlopsComponent(const GumbelSoftmaxFlopsComponent &other):
    RandomComponent(other),
    dim_(other.dim_),
    scale_(other.scale_),
    temp_proportion_(other.temp_proportion_) { }

Component* GumbelSoftmaxFlopsComponent::Copy() const {
  GumbelSoftmaxFlopsComponent *ans = new GumbelSoftmaxFlopsComponent(*this);
  return ans;
}

void GumbelSoftmaxFlopsComponent::Init(int32 dim, BaseFloat scale, BaseFloat temp_proportion) {
  temp_proportion_ = temp_proportion;
  scale_ = scale;
  dim_ = dim;
}

void GumbelSoftmaxFlopsComponent::InitFromConfig(ConfigLine *cfl) {
  int32 dim = 0;
  BaseFloat scale = 1.0;
  BaseFloat temp_proportion = 0.0;
  bool ok = cfl->GetValue("dim", &dim) &&
  ...
  //      KALDI_ERR << "Invalid initializer for layer of type "
  //                << Type() << ": \"" << cfl->WholeLine() << "\"";
  Init(dim, scale, temp_proportion);
}

std::string GumbelSoftmaxFlopsComponent::Info() const {
  std::ostringstream stream;
  stream << Type() << ", dim=" << dim_
         << ", scale=" << scale_
         << ", temp-proportion=" << temp_proportion_;
...
void* GumbelSoftmaxFlopsComponent::Propagate(const ComponentPrecomputedIndexes *indexes,
                                 const CuMatrixBase<BaseFloat> &in,
                                 CuMatrixBase<BaseFloat> *out) const {
  // Apply softmax function to each row of the output...
  // for that row, we do
...
void GumbelSoftmaxFlopsComponent::Backprop(const std::string &debug_info,
                                const ComponentPrecomputedIndexes *indexes,
                                const CuMatrixBase<BaseFloat> &in_value,
                                const CuMatrixBase<BaseFloat> &out_value,
                                const CuMatrixBase<BaseFloat> &out_deriv,
  ...
  // if (to_update_in) {
  //   GumbelSoftmaxFlopsComponent *to_update =
  //       dynamic_cast<GumbelSoftmaxFlopsComponent*>(to_update_in);
  //   to_update->StoreBackpropStats(out_deriv);
  // }
...
void GumbelSoftmaxFlopsComponent::Read(std::istream &is, bool binary) {
  std::string token;
  ReadToken(is, binary, &token);
  if (token == "<GumbelSoftmaxFlopsComponent>") {
    ReadToken(is, binary, &token);
  }
  KALDI_ASSERT(token == "<Dim>");
  ReadBasicType(is, binary, &dim_);  // read dimension.
  ReadToken(is, binary, &token);
  KALDI_ASSERT(token == "<Scale>");
  ReadBasicType(is, binary, &scale_);  // read dimension.
  ReadToken(is, binary, &token);
  KALDI_ASSERT(token == "<TempProportion>");
  ReadBasicType(is, binary, &temp_proportion_);  // read dropout rate
  ReadToken(is, binary, &token);
  KALDI_ASSERT(token == "</GumbelSoftmaxFlopsComponent>");
}

void GumbelSoftmaxFlopsComponent::Write(std::ostream &os, bool binary) const {
  WriteToken(os, binary, "<GumbelSoftmaxFlopsComponent>");
  WriteToken(os, binary, "<Dim>");
  WriteBasicType(os, binary, dim_);
  WriteToken(os, binary, "<Scale>");
  WriteBasicType(os, binary, scale_);
