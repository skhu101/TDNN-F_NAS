...
void SetGumbelTestMode(bool test_mode,  Nnet *nnet) {
  for (int32 c = 0; c < nnet->NumComponents(); c++) {
    Component *comp = nnet->GetComponent(c);
    TdnnDARTSV2Component *rc = dynamic_cast<TdnnDARTSV2Component*>(comp);
    if (rc != NULL)
      rc->SetTestMode(test_mode);
  }
}

void SetGaussianTestMode(bool test_mode,  Nnet *nnet) {
  for (int32 c = 0; c < nnet->NumComponents(); c++) {
    Component *comp = nnet->GetComponent(c);
    BayesTdnnComponent *rc = dynamic_cast<BayesTdnnComponent*>(comp);
    if (rc != NULL)
      rc->SetTestMode(test_mode);
  }
}
  ...
  for (size_t i = 0; i < config_lines.size(); i++) {
    ...
    if (directive == "convert-to-fixed-affine") {
      ...
      for (int32 c = 0; c < nnet->NumComponents(); c++) {
        if (NameMatchesPattern(nnet->GetComponentName(c).c_str(),
                               name_pattern.c_str())) {
          TimeHeightConvolutionDARTSComponent *timeheightconvolutiondartscomponent =
             dynamic_cast<TimeHeightConvolutionDARTSComponent*>(nnet->GetComponent(c));
          TdnnDARTSV2Component *tdnndartsv2component =
             dynamic_cast<TdnnDARTSV2Component*>(nnet->GetComponent(c));
          TdnnDARTSV3Component *tdnndartsv3component =
             dynamic_cast<TdnnDARTSV3Component*>(nnet->GetComponent(c));
          TdnnDARTSV3SelectComponent *tdnndartsv3selectcomponent =
             dynamic_cast<TdnnDARTSV3SelectComponent*>(nnet->GetComponent(c));
          GumbelSoftmaxComponent *gumbelsoftmaxcomponent =
          ...
          BayesTdnnComponent *bayestdnncomponent =
             dynamic_cast<BayesTdnnComponent*>(nnet->GetComponent(c));
          if (tdnndartsv2component != NULL) {
            tdnndartsv2component->SetTempProportion(proportion);
            num_temp_proportions_set++;
          } else if (tdnndartsv3component != NULL) {
            tdnndartsv3component->SetTempProportion(proportion);
            num_temp_proportions_set++;
          } else if (tdnndartsv3selectcomponent != NULL) {
            tdnndartsv3selectcomponent->SetTempProportion(proportion);
            num_temp_proportions_set++;
          } else if (gumbelsoftmaxcomponent != NULL) {
            gumbelsoftmaxcomponent->SetTempProportion(proportion);
            num_temp_proportions_set++;
          } else if (gumbelsoftmaxflopscomponent != NULL) {
            gumbelsoftmaxflopscomponent->SetTempProportion(proportion);
            num_temp_proportions_set++;
          } else if (bayestdnncomponent != NULL) {
            bayestdnncomponent->SetTempProportion(proportion);
            num_temp_proportions_set++;
          } else if (timeheightconvolutiondartscomponent != NULL) {
            timeheightconvolutiondartscomponent->SetTempProportion(proportion);
            num_temp_proportions_set++;
          }
        }
      }
