...
cd /project_bdda3/bdda/skhu/toolkits/kaldi/egs/swbd_wav/s5c
...
if [ $stage -le 0 ]; then
  local/swbd1_data_download.sh kaldi/egs/swbd_wav/LDC/LDC97S62
...
if [ $stage -le 2 ]; then
  ...
  local/swbd1_data_prep.sh kaldi/egs/swbd_wav/LDC/LDC97S62
...
if [ $stage -le 3 ]; then
  ...
  fisher_dirs="kaldi/egs/swbd_wav/LDC/LDC2004T19/fe_03_p1_tran/ kaldi/egs/swbd_wav/LDC/LDC2005T19/fe_03_p2_tran/"
...
if [ $stage -le 4 ]; then
  ...
  utils/format_lm_sri.sh --srilm-opts "$srilm_opts" data/lang_nosp $LM data/local/dict_nosp/lexicon.txt data/lang_nosp_sw1_tg
...
if [ $stage -le 5 ]; then
  ...
  local/eval2000_data_prep.sh kaldi/egs/swbd_wav/LDC/hub5e_00 kaldi/egs/swbd_wav/LDC/2000_hub5_eng_eval_tr
fi
...
if [ $stage -le 9 ]; then
  ...
  steps/train_mono.sh --nj 30 --cmd "$train_cmd" data/train_30kshort data/lang_nosp exp/mono
fi

if [ $stage -le 10 ]; then
  steps/align_si.sh --nj 30 --cmd "$train_cmd" data/train_100k_nodup data/lang_nosp exp/mono exp/mono_ali

  steps/train_deltas.sh --cmd "$train_cmd" 3200 30000 data/train_100k_nodup data/lang_nosp exp/mono_ali exp/tri1
...
if [ $stage -le 11 ]; then
  steps/align_si.sh --nj 30 --cmd "$train_cmd" data/train_100k_nodup data/lang_nosp exp/tri1 exp/tri1_ali

  steps/train_deltas.sh --cmd "$train_cmd" 4000 70000 data/train_100k_nodup data/lang_nosp exp/tri1_ali exp/tri2
...
if [ $stage -le 12 ]; then
  ...
  steps/align_si.sh --nj 30 --cmd "$train_cmd" data/train_100k_nodup data/lang_nosp exp/tri2 exp/tri2_ali_100k_nodup

  # From now, we start using all of the data (except some duplicates of common
  # utterances, which don't really contribute much).
  steps/align_si.sh --nj 30 --cmd "$train_cmd" data/train_nodup data/lang_nosp exp/tri2 exp/tri2_ali_nodup

  # Do another iteration of LDA+MLLT training, on all the data.
  steps/train_lda_mllt.sh --cmd "$train_cmd" 6000 140000 data/train_nodup data/lang_nosp exp/tri2_ali_nodup exp/tri3
...
if [ $stage -le 13 ]; then
  ...
  LM=data/local/lm/sw1.o3g.kn.gz
  srilm_opts="-subset -prune-lowprobs -unk -tolower -order 3"
  utils/format_lm_sri.sh --srilm-opts "$srilm_opts" data/lang $LM data/local/dict/lexicon.txt data/lang_sw1_tg
...
if [ $stage -le 14 ]; then
  ...
  steps/align_fmllr.sh --nj 30 --cmd "$train_cmd" data/train_nodup data/lang exp/tri3 exp/tri3_ali_nodup


  steps/train_sat.sh  --cmd "$train_cmd" 11500 200000 data/train_nodup data/lang exp/tri3_ali_nodup exp/tri4

  graph_dir=exp/tri4/graph_sw1_tg

  $train_cmd $graph_dir/mkgraph.log utils/mkgraph.sh data/lang_sw1_tg exp/tri4 $graph_dir

  steps/decode_fmllr.sh --nj 30 --cmd "$decode_cmd" --config conf/decode.config $graph_dir data/eval2000 exp/tri4/decode_eval2000_sw1_tg

  steps/lmrescore_const_arpa.sh --cmd "$decode_cmd" data/lang_sw1_{tg,fg_htk_arpa} data/eval2000 exp/tri4/decode_eval2000_sw1_{tg,fg_htk_arpa}
...
if [ $stage -le 15 ]; then
  ...
  steps/align_fmllr.sh --nj 50 --cmd "$train_cmd" data/train_nodup data/lang exp/tri4 exp/tri4_ali_nodup
