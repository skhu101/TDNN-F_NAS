...
if [[ $parent_path =~ "95onehotpretrain_cvupdate_gumbel" ]]; then
    dir=exp/chain_NAS/tdnn_DARTS_bottleneckCBshare_95onehotpretrain_cvupdate_gumbel_flops_${flops_coef}_Child_Top${top_id}${affix}${suffix}
elif [[ $parent_path =~ "95onehotpretrain_cvupdate_softmax" ]]; then
    dir=exp/chain_NAS/tdnn_DARTS_bottleneckCBshare_95onehotpretrain_cvupdate_softmax_flops_${flops_coef}_Child_Top${top_id}${affix}${suffix}
elif [[ $parent_path =~ "gumbel" ]]; then
    dir=exp/chain_NAS/tdnn_DARTS_bottleneckCBshare_gumbel_Child_Top${top_id}${affix}${suffix}
elif [[ $parent_path =~ "softmax" ]]; then
    dir=exp/chain_NAS/tdnn_DARTS_bottleneckCBshare_softmax_Child_Top${top_id}${affix}${suffix}
fi

common_egs_dir=/project_bdda3/bdda/skhu/toolkits/kaldi/egs/swbd_wav/s5c/exp/chain/tdnn_fbk_40_iv_7q_sp/egs
