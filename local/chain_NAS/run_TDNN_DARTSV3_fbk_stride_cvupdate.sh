...
dir=exp/chain_NAS/tdnn_DARTSV3_offset${offset_len}${affix}${suffix}_95onehotpretrain_cvupdate_softmax

if [ "$use_gumbel" = true ] ; then
    dir=exp/chain_NAS/tdnn_DARTSV3_offset${offset_len}${affix}${suffix}_95onehotpretrain_cvupdate_gumbel
fi
...
if [ $stage -le 12 ]; then
  ...
  if [ "$use_gumbel" = true ] ; then
      ...
      sed "s/<TdnnDARTSV3Component> <LearningRateFactor> 0/<TdnnDARTSV3Component> <LearningRateFactor> 0.0001/g" | \
      sed "s/<use-gumbel> F/<use-gumbel> T/g" | sed "s/<update-alpha> F/<update-alpha> T/g" | sed "s/<update-theta> T/<update-theta> F/g" | \
      sed "s/<uniform-sample> T/<uniform-sample> F/g" | \
      sed "s/<TestMode> F/<TestMode> T/g" | sed "s/BatchNormComponent/BatchNormTestComponent/g" | \
      nnet3-copy - $dir/0.raw
  else
      nnet3-am-copy --raw --binary=false --edits="set-learning-rate-factor learning-rate-factor=0" $parent_path/final.mdl - | \
      sed "s/<TdnnDARTSV3Component> <LearningRateFactor> 0/<TdnnDARTSV3Component> <LearningRateFactor> 0.0001/g" | \
      sed "s/<use-gumbel> F/<use-gumbel> F/g" | sed "s/<update-alpha> F/<update-alpha> T/g" | sed "s/<update-theta> T/<update-theta> F/g" | \
      sed "s/<uniform-sample> T/<uniform-sample> F/g" | \
      sed "s/<TestMode> F/<TestMode> T/g" | sed "s/BatchNormComponent/BatchNormTestComponent/g" | \
      nnet3-copy - $dir/0.raw
  fi
